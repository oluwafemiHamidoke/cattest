...
CREATE  Procedure [SISSEARCH].[MEDIAPRODUCTFAMILY_2_LOAD] 
As
/*---------------
Date: 10-10-2017
Object Description:  Loading changed data into SISSEARCH.MEDIAPRODUCTFAMILY_2 from base tables
...
Exec [SISSEARCH].[MEDIAPRODUCTFAMILY_2_LOAD]
Truncate table [SISSEARCH].[MEDIAPRODUCTFAMILY_2]
---------------*/
...
Select
	[BaseEngMediaNumber]
into #DeletedRecords
from  [SISSEARCH].[MEDIAPRODUCTFAMILY_2]
Except 
Select
...
Delete [SISSEARCH].[MEDIAPRODUCTFAMILY_2] 
From [SISSEARCH].[MEDIAPRODUCTFAMILY_2] a
...
SET @LAPSETIME = DATEDIFF(SS, @StepStartTime, GETDATE());
EXEC SISSEARCH.WriteLog @NAMEOFSPROC = @ProcName, @ISUPDATE = 1,@LAPSETIME = @LAPSETIME, @DATAVALUE = @RowCount, @LOGID = @StepLogID, 
@LOGMESSAGE =  'Deleted Records from Target [SISSEARCH].[MEDIAPRODUCTFAMILY_2]';

--Get Maximum insert date from Target
SET @StepStartTime= GETDATE()
EXEC @StepLogID = SISSEARCH.WriteLog @TIME = @StepStartTime, @NAMEOFSPROC = @ProcName;
	
Select  @LastInsertDate=coalesce(Max(InsertDate),'1900-01-01') From [SISSEARCH].[MEDIAPRODUCTFAMILY_2]
...
SET @LAPSETIME = DATEDIFF(SS, @StepStartTime, GETDATE());
EXEC SISSEARCH.WriteLog @NAMEOFSPROC = @ProcName, @ISUPDATE = 1,@LAPSETIME = @LAPSETIME, @DATAVALUE = @RowCount, @LOGID = @StepLogID, 
@LOGMESSAGE =  'Deleted Inserted Records from Target [SISSEARCH].[MEDIAPRODUCTFAMILY_2]';
...
and b.TYPEINDICATOR = 'P' --Product Family
and not exists(
		select 1 from SISSEARCH.REF_SELECTIVE_EXCLUDEINFOTYPE 
	where INFOTYPEID = b.INFOTYPEID and [Excluded_Values] = a.MEDIAORIGIN
	)
...
INCLUDE (BASEENGLISHMEDIANUMBER,ProductCode,[InsertDate])
...
Insert into [SISSEARCH].[MEDIAPRODUCTFAMILY_2] ([BaseEngMediaNumber],ID,ProductCode,InsertDate)
Select  
    a.BASEENGLISHMEDIANUMBER,
	a.ID,
...
SET @LAPSETIME = DATEDIFF(SS, @StepStartTime, GETDATE());
EXEC SISSEARCH.WriteLog @NAMEOFSPROC = @ProcName, @ISUPDATE = 1,@LAPSETIME = @LAPSETIME, @DATAVALUE = @RowCount, @LOGID = @StepLogID, 
@LOGMESSAGE =  'Inserted into Target [SISSEARCH].[MEDIAPRODUCTFAMILY_2]';
