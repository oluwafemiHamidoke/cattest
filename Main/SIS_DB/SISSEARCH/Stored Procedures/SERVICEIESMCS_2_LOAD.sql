CREATE  Procedure [SISSEARCH].[SERVICEIESMCS_2_LOAD] 
As
/*---------------
Date: 20180504
Object Description:  Loading changed data into SISSEARCH.SERVICEIESMCS_2 from base tables
...
Exec [SISSEARCH].[SERVICEIESMCS_2_LOAD]
Truncate table [SISSEARCH].[SERVICEIESMCS_2]
---------------*/
...
into #DeletedRecords
from  [SISSEARCH].[SERVICEIESMCS_2]
Except 
Select
...
Delete [SISSEARCH].[SERVICEIESMCS_2] 
From [SISSEARCH].[SERVICEIESMCS_2] a
...
SET @LAPSETIME = DATEDIFF(SS, @StepStartTime, GETDATE());
EXEC SISSEARCH.WriteLog @NAMEOFSPROC = @ProcName, @ISUPDATE = 1,@LAPSETIME = @LAPSETIME, @DATAVALUE = @RowCount, @LOGID = @StepLogID, 
@LOGMESSAGE =  'Deleted Records from Target [SISSEARCH].[SERVICEIESMCS_2]';

--Get Maximum insert date from Target
SET @StepStartTime= GETDATE()
EXEC @StepLogID = SISSEARCH.WriteLog @TIME = @StepStartTime, @NAMEOFSPROC = @ProcName;
	
Select  @LastInsertDate=coalesce(Max(InsertDate),'1900-01-01') From [SISSEARCH].[SERVICEIESMCS_2]
...
SET @LAPSETIME = DATEDIFF(SS, @StepStartTime, GETDATE());
EXEC SISSEARCH.WriteLog @NAMEOFSPROC = @ProcName, @ISUPDATE = 1,@LAPSETIME = @LAPSETIME, @DATAVALUE = @RowCount, @LOGID = @StepLogID, 
@LOGMESSAGE =  'Deleted Inserted Records from Target [SISSEARCH].[SERVICEIESMCS_2]';
...
left outer join [SISWEB_OWNER].[LNKIEDATE] e on 
	a.IESYSTEMCONTROLNUMBER = e.IESYSTEMCONTROLNUMBER and
	e.LANGUAGEINDICATOR = 'E'
inner join [SISWEB_OWNER].[LNKIESMCS] f on 
	a.IESYSTEMCONTROLNUMBER = f.IESYSTEMCONTROLNUMBER and
	a.MEDIANUMBER = f.MEDIANUMBER
inner join [SISWEB_OWNER].[LNKIETITLE] d on 
	a.IESYSTEMCONTROLNUMBER = d.IESYSTEMCONTROLNUMBER and
	d.IELANGUAGEINDICATOR = 'E' and
	(d.GROUPTITLEINDICATOR='N' or d.GROUPTITLEINDICATOR is Null)
...
Insert into [SISSEARCH].[SERVICEIESMCS_2] 
(
	[ID]
	--,[BeginRange]
	--,[EndRange]
...
SET @LAPSETIME = DATEDIFF(SS, @StepStartTime, GETDATE());
EXEC SISSEARCH.WriteLog @NAMEOFSPROC = @ProcName, @ISUPDATE = 1,@LAPSETIME = @LAPSETIME, @DATAVALUE = @RowCount, @LOGID = @StepLogID, 
@LOGMESSAGE =  'Inserted into Target [SISSEARCH].[SERVICEIESMCS_2]';
