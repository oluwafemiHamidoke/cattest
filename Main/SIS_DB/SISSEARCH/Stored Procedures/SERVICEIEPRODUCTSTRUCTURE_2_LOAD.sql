CREATE  Procedure [SISSEARCH].[SERVICEIEPRODUCTSTRUCTURE_2_LOAD]
As
/*---------------
Date: 20180504
Object Description:  Loading changed data into SISSEARCH.SERVICEIEPRODUCTSTRUCTURE_2 from base tables
...
Exec [SISSEARCH].[SERVICEIEPRODUCTSTRUCTURE_2_LOAD]
Truncate table [SISSEARCH].[SERVICEIEPRODUCTSTRUCTURE_2]
---------------*/
...
into #DeletedRecords
from  [SISSEARCH].[SERVICEIEPRODUCTSTRUCTURE_2]
Except
Select Distinct
...
Delete [SISSEARCH].[SERVICEIEPRODUCTSTRUCTURE_2]
From [SISSEARCH].[SERVICEIEPRODUCTSTRUCTURE_2] a
...
SET @LAPSETIME = DATEDIFF(SS, @StepStartTime, GETDATE());
EXEC SISSEARCH.WriteLog @NAMEOFSPROC = @ProcName, @ISUPDATE = 1,@LAPSETIME = @LAPSETIME, @DATAVALUE = @RowCount, @LOGID = @StepLogID,
@LOGMESSAGE =  'Deleted Records from Target [SISSEARCH].[SERVICEIEPRODUCTSTRUCTURE_2]';

--Get Maximum insert date from Target
SET @StepStartTime= GETDATE()
EXEC @StepLogID = SISSEARCH.WriteLog @TIME = @StepStartTime, @NAMEOFSPROC = @ProcName;

Select  @LastInsertDate=coalesce(Max(InsertDate),'1900-01-01') From [SISSEARCH].[SERVICEIEPRODUCTSTRUCTURE_2]
...
SET @LAPSETIME = DATEDIFF(SS, @StepStartTime, GETDATE());
EXEC SISSEARCH.WriteLog @NAMEOFSPROC = @ProcName, @ISUPDATE = 1,@LAPSETIME = @LAPSETIME, @DATAVALUE = @RowCount, @LOGID = @StepLogID,
@LOGMESSAGE =  'Deleted Inserted Records from Target [SISSEARCH].[SERVICEIEPRODUCTSTRUCTURE_2]';
...
select a.IESYSTEMCONTROLNUMBER as [ID],
	...
	a.IESYSTEMCONTROLNUMBER,
	case
		when h.PARENTPRODUCTSTRUCTUREID <> '00000000' then h.PARENTPRODUCTSTRUCTUREID  -- per Lu 20190416 This should be h.PARENTPRODUCTSTRUCTUREID <> ‘00000000’ then h.PARENTPRODUCTSTRUCTUREID
		else g.PSID
    end as [SystemPSID],
...
left outer join [SISWEB_OWNER].[LNKIEDATE] e on
	a.IESYSTEMCONTROLNUMBER = e.IESYSTEMCONTROLNUMBER and
	e.LANGUAGEINDICATOR = 'E'
inner join [SISWEB_OWNER_SHADOW].[LNKIEPSID] g on
	a.IESYSTEMCONTROLNUMBER = g.IESYSTEMCONTROLNUMBER and
	a.MEDIANUMBER = g.MEDIANUMBER
inner join [SISWEB_OWNER].[MASPRODUCTSTRUCTURE] h on
	g.PSID = h.PRODUCTSTRUCTUREID and
	h.LANGUAGEINDICATOR = 'E'
inner join [SISWEB_OWNER].[LNKIETITLE] d on
	a.IESYSTEMCONTROLNUMBER = d.IESYSTEMCONTROLNUMBER and
	d.IELANGUAGEINDICATOR = 'E' and
	(d.GROUPTITLEINDICATOR='N' or d.GROUPTITLEINDICATOR is NULL)
...
case
	when h.PARENTPRODUCTSTRUCTUREID <> '00000000' then h.PARENTPRODUCTSTRUCTUREID
	else g.PSID
end
)
Select *
...
Insert into [SISSEARCH].[SERVICEIEPRODUCTSTRUCTURE_2]
(
	[ID]
	--,[BeginRange]
	--,[EndRange]
...
SET @LAPSETIME = DATEDIFF(SS, @StepStartTime, GETDATE());
EXEC SISSEARCH.WriteLog @NAMEOFSPROC = @ProcName, @ISUPDATE = 1,@LAPSETIME = @LAPSETIME, @DATAVALUE = @RowCount, @LOGID = @StepLogID,
@LOGMESSAGE =  'Inserted into Target [SISSEARCH].[SERVICEIEPRODUCTSTRUCTURE_2]';
