...
CREATE Procedure [SISSEARCH].[CONSOLIDATEDMEDIA_4_LOAD] as 
BEGIN 
BEGIN TRY

--Start
EXEC SISSEARCH.WriteLog @NAMEOFSPROC = 'SISSEARCH.CONSOLIDATEDMEDIA_4_LOAD', @LOGMESSAGE = 'Execution started';
...
--Var
Declare @Sproc varchar(200) = 'SISSEARCH.CONSOLIDATEDMEDIA_4_LOAD'
...
SELECT coalesce(bm.[ID], it.[ID], sn.[ID], pf.[ID], ph.[ID]) ID
      ,coalesce(bm.[BaseEngMediaNumber], it.[BaseEngMediaNumber], sn.[BaseEngMediaNumber], pf.[BaseEngMediaNumber], ph.[BaseEngMediaNumber]) [BaseEngMediaNumber]
	  ,GREATEST(bm.InsertDate, it.InsertDate, sn.InsertDate, pf.InsertDate, ph.[InsertDate]) [INSERTDATE]
	  --Basic Meda
      --,[BeginRange]
      --,[EndRange]
	  ,nullif(nullif(bm.[MediaNumber], '[""]'), '') [MediaNumber]
...
From [SISSEARCH].[BASICSERVICEMEDIA_2] bm
  Full outer join [SISSEARCH].[MEDIAINFOTYPE_2] it on bm.[ID] = it.[ID]
  Full outer join [SISSEARCH].[MEDIASNP_4] sn on coalesce(bm.[ID], it.[ID]) = sn.[ID]
  Full outer join [SISSEARCH].[MEDIAPRODUCTFAMILY_2] pf on coalesce(bm.[ID], it.[ID], sn.[ID]) = pf.[ID]
  full outer join [SISSEARCH].[MEDIAPRODUCTHIERARCHY_2] ph on coalesce(bm.[ID], it.[ID], sn.[ID], pf.[ID]) = ph.[ID]
...
SET @RowCount= @@RowCount
SET @LAPSETIME = DATEDIFF(SS, @TimeMarker, GETDATE());
EXEC SISSEARCH.WriteLog @NAMEOFSPROC = @Sproc, @LOGMESSAGE = 'Updated #Consolidated Suffixed IDs',@LAPSETIME = @LAPSETIME,@DATAVALUE = @RowCount;
...
--Delete from target
Delete SISSEARCH.CONSOLIDATEDMEDIA_4
From  SISSEARCH.CONSOLIDATEDMEDIA_4 t
Left outer join #Consolidated s on t.[ID] = s.[ID]
Where s.[ID] is null --Does not exist in source

SET @RowCount= @@RowCount
SET @LAPSETIME = DATEDIFF(SS, @TimeMarker, GETDATE());
EXEC SISSEARCH.WriteLog @NAMEOFSPROC = @Sproc, @LOGMESSAGE = 'Deleted records from SISSEARCH.CONSOLIDATEDMEDIA_4',@LAPSETIME = @LAPSETIME,@DATAVALUE = @RowCount;

 
--Select @RowCount, getdate(), 'Deleted records from SISSEARCH.CONSOLIDATEDMEDIA_4'
Set @TimeMarker = getdate()
...
Set 
	...
	,tgt.[familySubFamilySalesModelSNP] = src.[familySubFamilySalesModelSNP]
      ...
      ,tgt.[UpdatedDate_ru-RU] = src.[UpdatedDate_ru-RU]
	  ,tgt.[mediaOrigin] = src.[mediaOrigin]
From SISSEARCH.CONSOLIDATEDMEDIA_4 tgt
...
SET @RowCount= @@RowCount
SET @LAPSETIME = DATEDIFF(SS, @TimeMarker, GETDATE());
EXEC SISSEARCH.WriteLog @NAMEOFSPROC = @Sproc, @LOGMESSAGE = 'Updated records in SISSEARCH.CONSOLIDATEDMEDIA_4',@LAPSETIME = @LAPSETIME,@DATAVALUE = @RowCount;


--Select @RowCount, getdate(), 'Updated records in SISSEARCH.CONSOLIDATEDMEDIA_4'
Set @TimeMarker = getdate()

--Insert new ID's
INSERT SISSEARCH.CONSOLIDATEDMEDIA_4
([ID]
      ,[BaseEngMediaNumber]
      ,[INSERTDATE]
      ,[MediaNumber]
...
,s.[familySubFamilySalesModelSNP]
	  ...
	  ,s.[mediaOrigin]
From #Consolidated s
Left outer join SISSEARCH.CONSOLIDATEDMEDIA_4 t on s.[ID] = t.[ID]
Where t.[ID] is null --Does not exist in target

SET @RowCount= @@RowCount
SET @LAPSETIME = DATEDIFF(SS, @TimeMarker, GETDATE());
EXEC SISSEARCH.WriteLog @NAMEOFSPROC = @Sproc, @LOGMESSAGE = 'Inserted records in SISSEARCH.CONSOLIDATEDMEDIA_4',@LAPSETIME = @LAPSETIME,@DATAVALUE = @RowCount;

--Select @RowCount, getdate(), 'Inserted records in SISSEARCH.CONSOLIDATEDMEDIA_4'
Set @TimeMarker = getdate()

--Add Restriction Codes
Update c 
Set [RestrictionCode] = nullif(nullif(r.[RestrictionCode], '[""]'), '')
From [SISSEARCH].[CONSOLIDATEDMEDIA_4] c
Inner join [SISSEARCH].[MEDIARESTRICTACCESS_2] r on c.BaseEngMediaNumber = r.BaseEngMediaNumber
Where isnull(c.RestrictionCode, '') <> r.RestrictionCode

SET @RowCount= @@RowCount
SET @LAPSETIME = DATEDIFF(SS, @TimeMarker, GETDATE());
EXEC SISSEARCH.WriteLog @NAMEOFSPROC = @Sproc, @LOGMESSAGE = 'Updated SISSEARCH.CONSOLIDATEDMEDIA_4.RestrictionCode',@LAPSETIME = @LAPSETIME,@DATAVALUE = @RowCount;
...
EXEC SISSEARCH.WriteLog @NAMEOFSPROC = 'SISSEARCH.CONSOLIDATEDMEDIA_4_LOAD', @LOGMESSAGE = @ERRORMESSAGE;
