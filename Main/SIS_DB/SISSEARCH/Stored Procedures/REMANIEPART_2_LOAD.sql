CREATE  Procedure [SISSEARCH].[REMANIEPART_2_LOAD] 
As
/*---------------
Date: 10-04-2017
Object Description:  Loading changed data into SISSEARCH.REMANIEPART_2 from base tables
Exec [SISSEARCH].[REMANIEPART_2_LOAD]
Truncate table [SISSEARCH].[REMANIEPART_2]
---------------*/
...
SET @LAPSETIME = DATEDIFF(SS, @StepStartTime, GETDATE());
EXEC SISSEARCH.WriteLog @NAMEOFSPROC = @ProcName, @ISUPDATE = 1,@LAPSETIME = @LAPSETIME, @DATAVALUE = @RowCount, @LOGID = @StepLogID, 
@LOGMESSAGE =  'Deleted Records from Target [SISSEARCH].[REMANIEPART_2] ';
...
Select  @LASTINSERTDATE=coalesce(Max(INSERTDATE),'1900-01-01') From [SISSEARCH].[REMANIEPART_2]
...
Delete from [SISSEARCH].[REMANIEPART_2]
...
SET @LAPSETIME = DATEDIFF(SS, @StepStartTime, GETDATE());
EXEC SISSEARCH.WriteLog @NAMEOFSPROC = @ProcName, @ISUPDATE = 1,@LAPSETIME = @LAPSETIME, @DATAVALUE = @RowCount, @LOGID = @StepLogID, 
@LOGMESSAGE =  'Deleted Inserted Records from Target [SISSEARCH].[REMANIEPART_2]';
...
inner join [sis].[vw_RelatedPartsNoKits] k
	on k.PARTNUMBER = b.IEPARTNUMBER and k.TYPEINDICATOR = 'R'
--Where a.SNPType = 'P'
)
Select * 
...
Select  
	...
	a.ID,
	coalesce(f.REMANIEPARTString,'') As REMANIEPART,
...
Insert into [SISSEARCH].[REMANIEPART_2] (IESYSTEMCONTROLNUMBER,ID,REMANIEPART,INSERTDATE)
Select IESYSTEMCONTROLNUMBER,ID,REMANIEPART,INSERTDATE from #RecordsToStringResult
...
SET @LAPSETIME = DATEDIFF(SS, @StepStartTime, GETDATE());
EXEC SISSEARCH.WriteLog @NAMEOFSPROC = @ProcName, @ISUPDATE = 1,@LAPSETIME = @LAPSETIME, @DATAVALUE = @RowCount, @LOGID = @StepLogID, 
@LOGMESSAGE =  'Insert into Target [SISSEARCH].[REMANIEPART_2]';
...
Insert into [SISSEARCH].[REMANIEPART_2] (IESYSTEMCONTROLNUMBER,ID,REMANIEPART,INSERTDATE)
Select IESYSTEMCONTROLNUMBER,ID,REMANIEPART,INSERTDATE from #RecordsToStringResult_C
