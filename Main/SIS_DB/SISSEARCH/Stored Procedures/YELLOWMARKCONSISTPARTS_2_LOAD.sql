CREATE  Procedure [SISSEARCH].[YELLOWMARKCONSISTPARTS_2_LOAD] 
As
/*---------------
Date: 10-04-2017
Object Description:  Loading changed data into SISSEARCH.YELLOWMARKCONSISTPARTS_2 from base tables
Exec [SISSEARCH].[YELLOWMARKCONSISTPARTS_2_LOAD]
Truncate table [SISSEARCH].[YELLOWMARKCONSISTPARTS_2]
---------------*/
...
into #DeletedRecords
from  [SISSEARCH].[YELLOWMARKCONSISTPARTS_2]
Except 
Select IESYSTEMCONTROLNUMBER
...
Delete from 
[SISSEARCH].[YELLOWMARKCONSISTPARTS_2] 
...
SET @LAPSETIME = DATEDIFF(SS, @StepStartTime, GETDATE());
EXEC SISSEARCH.WriteLog @NAMEOFSPROC = @ProcName, @ISUPDATE = 1,@LAPSETIME = @LAPSETIME, @DATAVALUE = @RowCount, @LOGID = @StepLogID, 
@LOGMESSAGE =  'Deleted Records from Target [SISSEARCH].[YELLOWMARKCONSISTPARTS_2] ';
...
Select  @LASTINSERTDATE=coalesce(Max(INSERTDATE),'1900-01-01') From [SISSEARCH].[YELLOWMARKCONSISTPARTS_2]
...
Delete from [SISSEARCH].[YELLOWMARKCONSISTPARTS_2]
...
SET @LAPSETIME = DATEDIFF(SS, @StepStartTime, GETDATE());
EXEC SISSEARCH.WriteLog @NAMEOFSPROC = @ProcName, @ISUPDATE = 1,@LAPSETIME = @LAPSETIME, @DATAVALUE = @RowCount, @LOGID = @StepLogID, 
@LOGMESSAGE =  'Deleted Inserted Records from Target [SISSEARCH].[YELLOWMARKCONSISTPARTS_2]';
...
;with CTE_YELLOWMARKCONSISTPART as
(
...
From CTE_YELLOWMARKCONSISTPART;
...
INCLUDE ([IESYSTEMCONTROLNUMBER],[YELLOWMARKCONSISTPART],[INSERTDATE])
...
Select  
	...
	a.ID,
	coalesce(f.YELLOWMARKCONSISTPARTString,'') As YELLOWMARKCONSISTPART,
...
(
	SELECT  '[' + stuff
	(
		(SELECT '","' + cast(YELLOWMARKCONSISTPART as varchar(MAX))        
		FROM #RecordsToString as b 
		where a.ID=b.ID 
		order by b.ID
		FOR XML PATH(''), TYPE).value('.', 'varchar(max)')   
	,1,2,'') + '"'+']'     
) f (YELLOWMARKCONSISTPARTString)
Group by 
a.IESYSTEMCONTROLNUMBER,
a.ID,
f.YELLOWMARKCONSISTPARTString
...
Insert into [SISSEARCH].[YELLOWMARKCONSISTPARTS_2] (IESYSTEMCONTROLNUMBER,ID,YELLOWMARKCONSISTPART,INSERTDATE)
Select IESYSTEMCONTROLNUMBER,ID,YELLOWMARKCONSISTPART,INSERTDATE from #RecordsToStringResult
...
SET @LAPSETIME = DATEDIFF(SS, @StepStartTime, GETDATE());
EXEC SISSEARCH.WriteLog @NAMEOFSPROC = @ProcName, @ISUPDATE = 1,@LAPSETIME = @LAPSETIME, @DATAVALUE = @RowCount, @LOGID = @StepLogID, 
@LOGMESSAGE =  'Insert into Target [SISSEARCH].[YELLOWMARKCONSISTPARTS_2]';
...
Insert into [SISSEARCH].[YELLOWMARKCONSISTPARTS_2] (IESYSTEMCONTROLNUMBER,ID,YELLOWMARKCONSISTPART,INSERTDATE)
Select IESYSTEMCONTROLNUMBER,ID,YELLOWMARKCONSISTPART,INSERTDATE from #RecordsToStringResult_C
