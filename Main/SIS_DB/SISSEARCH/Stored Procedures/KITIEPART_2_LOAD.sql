CREATE  Procedure [SISSEARCH].[KITIEPART_2_LOAD] 
As
/*---------------
Date: 10-04-2017
Object Description:  Loading changed data into SISSEARCH.KITIEPART_2 from base tables
Exec [SISSEARCH].[KITIEPART_2_LOAD]
Truncate table [SISSEARCH].[KITIEPART_2]
---------------*/
...
SET @LAPSETIME = DATEDIFF(SS, @StepStartTime, GETDATE());
EXEC SISSEARCH.WriteLog @NAMEOFSPROC = @ProcName, @ISUPDATE = 1,@LAPSETIME = @LAPSETIME, @DATAVALUE = @RowCount, @LOGID = @StepLogID, 
@LOGMESSAGE =  'Deleted Records from Target [SISSEARCH].[KITIEPART_2] ';
...
Select  @LASTINSERTDATE=coalesce(Max(INSERTDATE),'1900-01-01') From [SISSEARCH].[KITIEPART_2]
...
Delete from [SISSEARCH].[KITIEPART_2]
...
SET @LAPSETIME = DATEDIFF(SS, @StepStartTime, GETDATE());
EXEC SISSEARCH.WriteLog @NAMEOFSPROC = @ProcName, @ISUPDATE = 1,@LAPSETIME = @LAPSETIME, @DATAVALUE = @RowCount, @LOGID = @StepLogID, 
@LOGMESSAGE =  'Deleted Inserted Records from Target [SISSEARCH].[KITIEPART_2]';
							...
							replace( --replace horizontal tab (ascii 09)
								replace( --replace backspace (ascii 08)
									replace( --escape \
										...
										isnull(k.PARENTPART ,'')+':'+isnull(k.KITNUMBER,'')+':'+isnull(g.PARTNAME, '')
									,'\', '\\')
								,char(8), ' ')
							,char(9), ' ')
						,char(10), ' ')
...
inner join KIM.SIS_KitParentParts k
	on k.PARENTPART = b.IEPARTNUMBER
inner join KIM.[SIS_KitNumbers] g
	on k.KITNUMBER = g.KITNUMBER
--Where a.SNPType = 'P'
)
Select * 
...
Insert into [SISSEARCH].[KITIEPART_2] (IESYSTEMCONTROLNUMBER,ID,KITIEPART,INSERTDATE)
...
SET @LAPSETIME = DATEDIFF(SS, @StepStartTime, GETDATE());
EXEC SISSEARCH.WriteLog @NAMEOFSPROC = @ProcName, @ISUPDATE = 1,@LAPSETIME = @LAPSETIME, @DATAVALUE = @RowCount, @LOGID = @StepLogID, 
@LOGMESSAGE =  'Insert into Target [SISSEARCH].[KITIEPART_2]';
...
Insert into [SISSEARCH].[KITIEPART_2] (IESYSTEMCONTROLNUMBER,ID,KITIEPART,INSERTDATE)
Select IESYSTEMCONTROLNUMBER,ID,KITIEPART,INSERTDATE from #RecordsToStringResult_C
...
EXEC @StepLogID = SISSEARCH.WriteLog @TIME = @StepStartTime,@LOGTYPE = 'Error', @NAMEOFSPROC = @ProcName,@LOGMESSAGE = @error
