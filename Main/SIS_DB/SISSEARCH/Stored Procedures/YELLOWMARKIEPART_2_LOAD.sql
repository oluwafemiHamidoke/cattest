CREATE  Procedure [SISSEARCH].[YELLOWMARKIEPART_2_LOAD] 
As
/*---------------
Date: 10-04-2017
Object Description:  Loading changed data into SISSEARCH.YELLOWMARKIEPART_2 from base tables
Exec [SISSEARCH].[YELLOWMARKIEPART_2_LOAD]
Truncate table [SISSEARCH].[YELLOWMARKIEPART_2]
---------------*/
...
into #DeletedRecords
from  [SISSEARCH].[YELLOWMARKIEPART_2]
Except 
Select IESYSTEMCONTROLNUMBER
...
SET @LAPSETIME = DATEDIFF(SS, @StepStartTime, GETDATE());
EXEC SISSEARCH.WriteLog @NAMEOFSPROC = @ProcName, @ISUPDATE = 1,@LAPSETIME = @LAPSETIME, @DATAVALUE = @RowCount, @LOGID = @StepLogID, 
@LOGMESSAGE =  'Deleted Records from Target [SISSEARCH].[YELLOWMARKIEPART_2] ';
...
Select  @LASTINSERTDATE=coalesce(Max(INSERTDATE),'1900-01-01') From [SISSEARCH].[YELLOWMARKIEPART_2]
...
Delete from [SISSEARCH].[YELLOWMARKIEPART_2]
...
SET @LAPSETIME = DATEDIFF(SS, @StepStartTime, GETDATE());
EXEC SISSEARCH.WriteLog @NAMEOFSPROC = @ProcName, @ISUPDATE = 1,@LAPSETIME = @LAPSETIME, @DATAVALUE = @RowCount, @LOGID = @StepLogID, 
@LOGMESSAGE =  'Deleted Inserted Records from Target [SISSEARCH].[YELLOWMARKIEPART_2]';
...
inner join [sis].[vw_RelatedPartsNoKits] k
	on k.PARTNUMBER = b.IEPARTNUMBER and k.TYPEINDICATOR in ('CL', 'Y')
--Where a.SNPType = 'P'
)
Select * 
...
Insert into [SISSEARCH].[YELLOWMARKIEPART_2] (IESYSTEMCONTROLNUMBER,ID,YELLOWMARKIEPART,INSERTDATE)
...
SET @LAPSETIME = DATEDIFF(SS, @StepStartTime, GETDATE());
EXEC SISSEARCH.WriteLog @NAMEOFSPROC = @ProcName, @ISUPDATE = 1,@LAPSETIME = @LAPSETIME, @DATAVALUE = @RowCount, @LOGID = @StepLogID, 
@LOGMESSAGE =  'Insert into Target [SISSEARCH].[YELLOWMARKIEPART_2]';
...
Insert into [SISSEARCH].[YELLOWMARKIEPART_2] (IESYSTEMCONTROLNUMBER,ID,YELLOWMARKIEPART,INSERTDATE)
Select IESYSTEMCONTROLNUMBER,ID,YELLOWMARKIEPART,INSERTDATE from #RecordsToStringResult_C
