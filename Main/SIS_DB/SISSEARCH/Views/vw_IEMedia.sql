...
--CREATE or ALTER VIEW SISSEARCH.vw_IEMedia_Origin AS
--select distinct e.IE_ID, 
...
CREATE   VIEW SISSEARCH.vw_IEMedia AS
select IE_ID, 
	'["' + string_agg(cast(Media_Number as nvarchar(max)), '","') + '"]' as MediaNumbers,
	'["' + string_agg(cast(PIPPS_Number as nvarchar(max)), '","') + '"]' as PIPPS_Number
from [SISSEARCH].[vw_IEMedia_Origin] mo
GROUP BY IE_ID
GO
