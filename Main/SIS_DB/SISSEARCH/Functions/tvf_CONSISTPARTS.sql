CREATE FUNCTION [SISSEARCH].[tvf_CONSISTPARTS](@DATE DATETIME)
	RETURNS TABLE
AS RETURN SELECT 
		b.IESYSTEMCONTROLNUMBER,
		b.IESYSTEMCONTROLNUMBER as ID,
		max(try_cast(b.[LASTMODIFIEDDATE] as datetime)) INSERTDATE,
...
from [SISSEARCH].[tvf_CONSISTPARTSORIGIN](@DATE) b
LEFT JOIN [SISSEARCH].[vw_TRANSLATEDPARTS] t on b.PARTNAME = t.PARTNAME
