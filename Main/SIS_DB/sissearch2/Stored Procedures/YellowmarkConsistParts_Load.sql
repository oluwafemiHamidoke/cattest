CREATE  Procedure [sissearch2].[YellowmarkConsistParts_Load] 
As
...
into #DeletedRecords
from [sissearch2].[YellowmarkConsistParts]
Except 
Select IESystemControlNumber as IESYSTEMCONTROLNUMBER
...
Delete from 
[sissearch2].[YellowmarkConsistParts] 
...
SET @LAPSETIME = DATEDIFF(SS, @StepStartTime, GETDATE());
EXEC sissearch2.WriteLog @NAMEOFSPROC = @ProcName, @ISUPDATE = 1,@LAPSETIME = @LAPSETIME, @DATAVALUE = @RowCount, @LOGID = @StepLogID, 
@LOGMESSAGE =  'Deleted Records from Target [sissearch2].[YellowmarkConsistParts] ';
...
Select  @LASTINSERTDATE=coalesce(Max(INSERTDATE),'1900-01-01') From [sissearch2].[YellowmarkConsistParts]
...
Delete from [sissearch2].[YellowmarkConsistParts]
...
SET @LAPSETIME = DATEDIFF(SS, @StepStartTime, GETDATE());
EXEC sissearch2.WriteLog @NAMEOFSPROC = @ProcName, @ISUPDATE = 1,@LAPSETIME = @LAPSETIME, @DATAVALUE = @RowCount, @LOGID = @StepLogID, 
@LOGMESSAGE =  'Deleted Inserted Records from Target [sissearch2].[YellowmarkConsistParts]';
						...
						replace( --replace line feed (ascii 10)
							replace( --replace horizontal tab (ascii 09)
								replace( --replace backspace (ascii 08)
									replace( --escape \
										isnull(e.Part_Number,'')+':'+isnull(e.Related_Part_Number,'')+':'+isnull(e.Related_Part_Name, '')
									,'\', '\\')
								,char(8), ' ')
							,char(9), ' ')
						,char(10), ' ')
...
inner join sis.Part_IEPart_Relation h on f.IEPart_ID=h.IEPart_ID
inner join sis.Part b on b.Part_ID=h.Part_ID 
...
Insert into [sissearch2].[YellowmarkConsistParts] (IESYSTEMCONTROLNUMBER,ID,YELLOWMARKCONSISTPART,INSERTDATE)
...
SET @LAPSETIME = DATEDIFF(SS, @StepStartTime, GETDATE());
EXEC sissearch2.WriteLog @NAMEOFSPROC = @ProcName, @ISUPDATE = 1,@LAPSETIME = @LAPSETIME, @DATAVALUE = @RowCount, @LOGID = @StepLogID, 
@LOGMESSAGE =  'Insert into Target [sissearch2].[YellowmarkConsistParts]';
