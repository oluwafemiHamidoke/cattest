...
-- Description: Conditional load from STAGING LNKMEDIASNP
-- =============================================
CREATE PROCEDURE SISWEB_OWNER_STAGING.LNKMEDIASNP_Insert_Update_Delete (@FORCE_LOAD BIT = 'FALSE'
																	   ,@DEBUG      BIT = 'FALSE') 
AS
BEGIN
...
*/
		...
		SELECT CT.SYS_CHANGE_OPERATION AS OPERATION,CT.MEDIANUMBER AS MEDIANUMBER,CT.SNP AS SNP,CT.BEGINNINGRANGE AS BEGINNINGRANGE,CT.ENDRANGE AS ENDRANGE,CT.INFOTYPEID AS
		INFOTYPEID,CT.TYPEINDICATOR AS TYPEINDICATOR,CT.SYS_CHANGE_VERSION AS SYS_CHANGE_VERSION
		--,CT.SYS_CHANGE_COLUMNS AS   SYS_CHANGE_COLUMNS -- despita having LASTMODIFIEDDATE, this is not needed, we disregard the UPDATE altogether, all columns are in PK
		INTO #DIFF_PRIMARY_KEYS
		FROM CHANGETABLE(CHANGES SISWEB_OWNER_STAGING.LNKMEDIASNP,@LAST_SYNCHRONIZATION_VERSION) AS CT;
		...
		BEGIN
			...
			BEGIN
				...
				IF @ROWCOUNT > 0
				BEGIN
					DELETE FROM SISWEB_OWNER.LNKMEDIASNP
					OUTPUT 'DELETE' ACTIONTYPE,DELETED.MEDIANUMBER,DELETED.SNP,DELETED.BEGINNINGRANGE,DELETED.ENDRANGE,DELETED.INFOTYPEID,DELETED.TYPEINDICATOR,DELETED.
					LASTMODIFIEDDATE
						   INTO @MERGE_RESULTS
					WHERE EXISTS
					(
						...
						WHERE DK.MEDIANUMBER = LNKMEDIASNP.MEDIANUMBER AND 
							  DK.SNP = LNKMEDIASNP.SNP AND 
							  DK.BEGINNINGRANGE = LNKMEDIASNP.BEGINNINGRANGE AND 
							  DK.ENDRANGE = LNKMEDIASNP.ENDRANGE AND 
							  DK.INFOTYPEID = LNKMEDIASNP.INFOTYPEID AND 
							  DK.TYPEINDICATOR = LNKMEDIASNP.TYPEINDICATOR AND 
							  DK.OPERATION = 'D'
					);
				...
				IF @ROWCOUNT > 0
				BEGIN
					INSERT INTO SISWEB_OWNER.LNKMEDIASNP (MEDIANUMBER,SNP,BEGINNINGRANGE,ENDRANGE,INFOTYPEID,TYPEINDICATOR,LASTMODIFIEDDATE) 
					OUTPUT 'INSERT' ACTIONTYPE,INSERTED.MEDIANUMBER,INSERTED.SNP,INSERTED.BEGINNINGRANGE,INSERTED.ENDRANGE,INSERTED.INFOTYPEID,INSERTED.TYPEINDICATOR,INSERTED.
					LASTMODIFIEDDATE
						   INTO @MERGE_RESULTS
						   SELECT MEDIANUMBER,SNP,BEGINNINGRANGE,ENDRANGE,INFOTYPEID,TYPEINDICATOR,@SYSUTCDATETIME
						   FROM SISWEB_OWNER_STAGING.LNKMEDIASNP AS DT
						   WHERE EXISTS
						   (
							   SELECT *
							   FROM #DIFF_PRIMARY_KEYS AS DK
...
/* can't happen
				...
				SELECT @ROWCOUNT = COUNT(*) FROM #DIFF_PRIMARY_KEYS AS DK WHERE DK.OPERATION = 'U'; -- not needed all columns in PK
				--AND -- if the modified column is LASTMODIFIEDDATE ignore the update 
				--DK.SYS_CHANGE_COLUMNS <> 0x000000000A000000;
				IF @ROWCOUNT > 0
				BEGIN
					UPDATE SISWEB_OWNER.LNKMEDIASNP
					...
					OUTPUT 'UPDATE' ACTIONTYPE
						  ...
						  ,INSERTED.SNR
						   INTO @MERGE_RESULTS
					  FROM SISWEB_OWNER_STAGING.LNKMEDIASNP AS ST
						   JOIN SISWEB_OWNER.LNKMEDIASNP AS TT ON
...
*/
				...
				(
					...
					(
						SELECT MR.ACTIONTYPE,MR.MEDIANUMBER,MR.SNP,MR.BEGINNINGRANGE,MR.ENDRANGE,MR.INFOTYPEID,MR.TYPEINDICATOR,MR.LASTMODIFIEDDATE
		...
		BEGIN
			...
			/* MERGE command */
			MERGE INTO SISWEB_OWNER.LNKMEDIASNP tgt
			USING SISWEB_OWNER_STAGING.LNKMEDIASNP src
			ON src.MEDIANUMBER = tgt.MEDIANUMBER AND 
			   src.SNP = tgt.SNP AND 
			   src.BEGINNINGRANGE = tgt.BEGINNINGRANGE AND 
			   src.ENDRANGE = tgt.ENDRANGE AND 
			   src.INFOTYPEID = tgt.INFOTYPEID AND 
			   src.TYPEINDICATOR = tgt.TYPEINDICATOR
			--  -- can't happen
			...
			WHEN NOT MATCHED BY TARGET
				  THEN
				  INSERT(MEDIANUMBER,SNP,BEGINNINGRANGE,ENDRANGE,INFOTYPEID,TYPEINDICATOR,LASTMODIFIEDDATE)
				  VALUES (src.MEDIANUMBER,src.SNP,src.BEGINNINGRANGE,src.ENDRANGE,src.INFOTYPEID,src.TYPEINDICATOR,@SYSUTCDATETIME) 
			WHEN NOT MATCHED BY SOURCE
				  THEN DELETE
			OUTPUT $ACTION,COALESCE(inserted.MEDIANUMBER,deleted.MEDIANUMBER) MEDIANUMBER,COALESCE(inserted.SNP,deleted.SNP) SNP,COALESCE(inserted.BEGINNINGRANGE,deleted.
			BEGINNINGRANGE) BEGINNINGRANGE,COALESCE(inserted.ENDRANGE,deleted.ENDRANGE) ENDRANGE,COALESCE(inserted.INFOTYPEID,deleted.INFOTYPEID) INFOTYPEID,COALESCE(inserted.
			TYPEINDICATOR,deleted.TYPEINDICATOR) TYPEINDICATOR,COALESCE(inserted.LASTMODIFIEDDATE,deleted.LASTMODIFIEDDATE) LASTMODIFIEDDATE
				   INTO @MERGE_RESULTS;
		...
		COMMIT;
								...
								UPDATE STATISTICS SISWEB_OWNER.LNKMEDIASNP WITH FULLSCAN;
