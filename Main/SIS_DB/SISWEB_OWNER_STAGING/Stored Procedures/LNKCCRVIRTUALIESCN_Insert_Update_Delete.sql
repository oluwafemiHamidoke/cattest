...
-- Modify Date: 
-- Description: Conditional load from STAGING for LNKCCRVIRTUALIESCN
-- =============================================

CREATE PROCEDURE SISWEB_OWNER_STAGING.LNKCCRVIRTUALIESCN_Insert_Update_Delete (@FORCE_LOAD BIT = 'FALSE'
                                                                               ,@DEBUG      BIT = 'FALSE') 
AS
BEGIN
    ...
    BEGIN TRY
        DECLARE @SCHEMANAME sysname = 'SISWEB_OWNER_STAGING'
               ,@TABLENAME  sysname = 'LNKCCRVIRTUALIESCN';
        ...
        FROM CHANGETABLE(CHANGES SISWEB_OWNER_STAGING.LNKCCRVIRTUALIESCN,@LAST_SYNCHRONIZATION_VERSION) AS CT;
        ...
        BEGIN
            ...
            BEGIN
                ...
                IF @ROWCOUNT > 0
                BEGIN
                    DELETE FROM SISWEB_OWNER.LNKCCRVIRTUALIESCN
                    ...
                    (
                        ...
                        WHERE DK.SNP = LNKCCRVIRTUALIESCN.SNP AND
                              DK.OPERATIONRID = LNKCCRVIRTUALIESCN.OPERATIONRID AND
                              DK.IESYSTEMCONTROLNUMBER = LNKCCRVIRTUALIESCN.IESYSTEMCONTROLNUMBER AND
                              DK.OPERATION = 'D'
                    );
                ...
                IF @ROWCOUNT > 0
                BEGIN
                    INSERT INTO SISWEB_OWNER.LNKCCRVIRTUALIESCN (SNP, OPERATIONRID, IESYSTEMCONTROLNUMBER, LASTMODIFIEDDATE) 
                    OUTPUT 'INSERT' ACTIONTYPE,INSERTED.SNP, INSERTED.OPERATIONRID, INSERTED.IESYSTEMCONTROLNUMBER, INSERTED.LASTMODIFIEDDATE
                           ...
                           FROM SISWEB_OWNER_STAGING.LNKCCRVIRTUALIESCN AS DT
                           WHERE EXISTS
                           (
                               SELECT 1
                               FROM #DIFF_PRIMARY_KEYS AS DK
                ...
                IF @ROWCOUNT > 0
                BEGIN
                    UPDATE SISWEB_OWNER.LNKCCRVIRTUALIESCN
                    ...
                    OUTPUT 'UPDATE' ACTIONTYPE,INSERTED.SNP, INSERTED.OPERATIONRID, INSERTED.IESYSTEMCONTROLNUMBER, INSERTED.LASTMODIFIEDDATE
                           INTO @MERGE_RESULTS
                    FROM SISWEB_OWNER_STAGING.LNKCCRVIRTUALIESCN AS ST
                         JOIN SISWEB_OWNER.LNKCCRVIRTUALIESCN AS TT ON ST.SNP = TT.SNP AND
        ...
        BEGIN
            ...
            /* MERGE command */
            MERGE INTO SISWEB_OWNER.LNKCCRVIRTUALIESCN tgt
            USING SISWEB_OWNER_STAGING.LNKCCRVIRTUALIESCN src
            ON
               src.SNP = tgt.SNP AND
        ...
        UPDATE STATISTICS SISWEB_OWNER.LNKCCRVIRTUALIESCN WITH FULLSCAN;
