...
-- Description: Conditional load from STAGING SIS2MEDIAEFFECTIVITY
-- =============================================
CREATE PROCEDURE SISWEB_OWNER_STAGING.SIS2MEDIAEFFECTIVITY_Insert_Update_Delete (@FORCE_LOAD BIT = 'FALSE'
																				,@DEBUG      BIT = 'FALSE') 
AS
BEGIN
	...
	BEGIN TRY
		DECLARE @SCHEMANAME sysname = 'SISWEB_OWNER_STAGING'
			   ,@TABLENAME  sysname = 'SIS2MEDIAEFFECTIVITY';
...
*/
		...
		SYS_CHANGE_VERSION
		--,CT.SYS_CHANGE_COLUMNS AS   SYS_CHANGE_COLUMNS -- not using this for the moment all columns in PK
		INTO #DIFF_PRIMARY_KEYS
		FROM CHANGETABLE(CHANGES SISWEB_OWNER_STAGING.SIS2MEDIAEFFECTIVITY,@LAST_SYNCHRONIZATION_VERSION) AS CT;
		...
		BEGIN
			...
			BEGIN
				...
				IF @ROWCOUNT > 0
				BEGIN
					DELETE FROM SISWEB_OWNER.SIS2MEDIAEFFECTIVITY
					OUTPUT 'DELETE' ACTIONTYPE,DELETED.MEDIANUMBER,DELETED.TYPEINDICATOR,DELETED.EFFECTIVITY,DELETED.LASTMODIFIEDDATE
						   INTO @MERGE_RESULTS
					WHERE EXISTS
					(
						SELECT *
						FROM #DIFF_PRIMARY_KEYS AS DK
						WHERE DK.MEDIANUMBER = SIS2MEDIAEFFECTIVITY.MEDIANUMBER AND 
							  DK.TYPEINDICATOR = SIS2MEDIAEFFECTIVITY.TYPEINDICATOR AND 
							  DK.EFFECTIVITY = SIS2MEDIAEFFECTIVITY.EFFECTIVITY AND 
							  DK.OPERATION = 'D'
					);
				...
				IF @ROWCOUNT > 0
				BEGIN
					INSERT INTO SISWEB_OWNER.SIS2MEDIAEFFECTIVITY (MEDIANUMBER,TYPEINDICATOR,EFFECTIVITY,LASTMODIFIEDDATE) 
					OUTPUT 'INSERT' ACTIONTYPE,INSERTED.MEDIANUMBER,INSERTED.TYPEINDICATOR,INSERTED.EFFECTIVITY,INSERTED.LASTMODIFIEDDATE
						   INTO @MERGE_RESULTS
						   SELECT MEDIANUMBER,TYPEINDICATOR,EFFECTIVITY,@SYSUTCDATETIME
						   FROM SISWEB_OWNER_STAGING.SIS2MEDIAEFFECTIVITY AS DT
						   WHERE EXISTS
						   (
							   SELECT *
							   FROM #DIFF_PRIMARY_KEYS AS DK
...
/*				-- can't happen, all columns in PK
				...
				SELECT @ROWCOUNT = COUNT(*) FROM #DIFF_PRIMARY_KEYS AS DK WHERE DK.OPERATION = 'U'; -- not used
				--AND -- if the modified column is LASTMODIFIEDDATE ignore the update 
				--DK.SYS_CHANGE_COLUMNS <> 0x000000000A000000;
				IF @ROWCOUNT > 0
				BEGIN
					UPDATE SISWEB_OWNER.SIS2MEDIAEFFECTIVITY
					...
					OUTPUT 'UPDATE' ACTIONTYPE
						  ...
						  ,INSERTED.SNR
						   INTO @MERGE_RESULTS
					  FROM SISWEB_OWNER_STAGING.SIS2MEDIAEFFECTIVITY AS ST
						   JOIN SISWEB_OWNER.SIS2MEDIAEFFECTIVITY AS TT ON
...
*/
				...
				(
					...
					(
						SELECT MR.ACTIONTYPE,MR.MEDIANUMBER,MR.TYPEINDICATOR,MR.EFFECTIVITY,MR.LASTMODIFIEDDATE
		...
		BEGIN
			...
			/* MERGE command */
			MERGE INTO SISWEB_OWNER.SIS2MEDIAEFFECTIVITY tgt
			USING SISWEB_OWNER_STAGING.SIS2MEDIAEFFECTIVITY src
			...
			WHEN NOT MATCHED BY TARGET
				  THEN
				  INSERT(MEDIANUMBER,TYPEINDICATOR,EFFECTIVITY,LASTMODIFIEDDATE)
				  VALUES (src.MEDIANUMBER,src.TYPEINDICATOR,src.EFFECTIVITY,@SYSUTCDATETIME) 
			...
			EFFECTIVITY,deleted.EFFECTIVITY) EFFECTIVITY,COALESCE(inserted.LASTMODIFIEDDATE,deleted.LASTMODIFIEDDATE) LASTMODIFIEDDATE
				   INTO @MERGE_RESULTS;
		...
		COMMIT;
				...
				UPDATE STATISTICS SISWEB_OWNER.SIS2MEDIAEFFECTIVITY WITH FULLSCAN;
