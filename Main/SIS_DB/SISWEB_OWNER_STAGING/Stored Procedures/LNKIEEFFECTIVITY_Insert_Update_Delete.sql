...
-- Description: Conditional load from STAGING LNKIEEFFECTIVITY
-- =============================================
CREATE PROCEDURE SISWEB_OWNER_STAGING.LNKIEEFFECTIVITY_Insert_Update_Delete (@FORCE_LOAD BIT = 'FALSE'
																			,@DEBUG      BIT = 'FALSE') 
AS
BEGIN
	...
	BEGIN TRY
		DECLARE @SCHEMANAME sysname = 'SISWEB_OWNER_STAGING'
			   ,@TABLENAME  sysname = 'LNKIEEFFECTIVITY';
...
*/
		...
		SELECT CT.SYS_CHANGE_OPERATION AS OPERATION,CT.MEDIANUMBER AS MEDIANUMBER,CT.IESCN AS IESCN,CT.TYPEINDICATOR AS TYPEINDICATOR,CT.EFFECTIVITY AS EFFECTIVITY,CT.
		SYS_CHANGE_VERSION AS SYS_CHANGE_VERSION
		INTO #DIFF_PRIMARY_KEYS
		FROM CHANGETABLE(CHANGES SISWEB_OWNER_STAGING.LNKIEEFFECTIVITY,@LAST_SYNCHRONIZATION_VERSION) AS CT;
		...
		BEGIN
			...
			BEGIN
				...
				IF @ROWCOUNT > 0
				BEGIN
					DELETE FROM SISWEB_OWNER.LNKIEEFFECTIVITY
					OUTPUT 'DELETE' ACTIONTYPE,DELETED.MEDIANUMBER,DELETED.IESCN,DELETED.TYPEINDICATOR,DELETED.EFFECTIVITY
						   INTO @MERGE_RESULTS
					WHERE EXISTS
					(
						SELECT *
						FROM #DIFF_PRIMARY_KEYS AS DK
						WHERE DK.MEDIANUMBER = LNKIEEFFECTIVITY.MEDIANUMBER AND 
							  DK.IESCN = LNKIEEFFECTIVITY.IESCN AND 
							  DK.TYPEINDICATOR = LNKIEEFFECTIVITY.TYPEINDICATOR AND 
							  DK.EFFECTIVITY = LNKIEEFFECTIVITY.EFFECTIVITY AND 
							  DK.OPERATION = 'D'
					);
				...
				IF @ROWCOUNT > 0
				BEGIN
					INSERT INTO SISWEB_OWNER.LNKIEEFFECTIVITY (MEDIANUMBER,IESCN,TYPEINDICATOR,EFFECTIVITY) 
					OUTPUT 'INSERT' ACTIONTYPE,INSERTED.MEDIANUMBER,INSERTED.IESCN,INSERTED.TYPEINDICATOR,INSERTED.EFFECTIVITY
						   INTO @MERGE_RESULTS
						   SELECT MEDIANUMBER,IESCN,TYPEINDICATOR,EFFECTIVITY
						   FROM SISWEB_OWNER_STAGING.LNKIEEFFECTIVITY AS DT
						   WHERE EXISTS
						   (
							   SELECT *
							   FROM #DIFF_PRIMARY_KEYS AS DK
							   WHERE DK.MEDIANUMBER = DT.MEDIANUMBER AND 
									 DK.IESCN = DT.IESCN AND 
...
/*
				-- 7.c Update if needed [CAN'T HAPPEN, ALL COLUMNS IN PK]
				SELECT @ROWCOUNT = COUNT(*) FROM #DIFF_PRIMARY_KEYS AS DK WHERE DK.OPERATION = 'U';
				IF @ROWCOUNT > 0
				BEGIN
					UPDATE SISWEB_OWNER.LNKIEEFFECTIVITY
					...
					OUTPUT 'UPDATE' ACTIONTYPE
						  ...
						  ,INSERTED.SNR
						   INTO @MERGE_RESULTS
					  FROM SISWEB_OWNER_STAGING.LNKIEEFFECTIVITY AS ST
						   JOIN SISWEB_OWNER.LNKIEEFFECTIVITY AS TT ON
...
*/
				...
				(
					...
					(
						SELECT MR.ACTIONTYPE,MR.MEDIANUMBER,MR.IESCN,MR.TYPEINDICATOR,MR.EFFECTIVITY
		...
		BEGIN
			...
			/* MERGE command */
			MERGE INTO SISWEB_OWNER.LNKIEEFFECTIVITY tgt
			USING SISWEB_OWNER_STAGING.LNKIEEFFECTIVITY src
			ON src.MEDIANUMBER = tgt.MEDIANUMBER AND 
			   src.IESCN = tgt.IESCN AND 
			...
			WHEN NOT MATCHED BY TARGET
				  THEN
				  INSERT(MEDIANUMBER,IESCN,TYPEINDICATOR,EFFECTIVITY)
				  VALUES (src.MEDIANUMBER,src.IESCN,src.TYPEINDICATOR,src.EFFECTIVITY) 
			WHEN NOT MATCHED BY SOURCE
				  THEN DELETE
			OUTPUT $ACTION,COALESCE(inserted.MEDIANUMBER,deleted.MEDIANUMBER) MEDIANUMBER,COALESCE(inserted.IESCN,deleted.IESCN) IESCN,COALESCE(inserted.TYPEINDICATOR,deleted.
			TYPEINDICATOR) TYPEINDICATOR,COALESCE(inserted.EFFECTIVITY,deleted.EFFECTIVITY) EFFECTIVITY
				   INTO @MERGE_RESULTS;
		...
		UPDATE STATISTICS SISWEB_OWNER.LNKIEEFFECTIVITY WITH FULLSCAN;
