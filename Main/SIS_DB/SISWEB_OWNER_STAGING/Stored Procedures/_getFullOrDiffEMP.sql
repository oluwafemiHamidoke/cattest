...
-- =============================================
CREATE PROCEDURE [SISWEB_OWNER_STAGING].[_getFullOrDiffEMP] (@SCHEMANAME      NVARCHAR(128)
...
--	BEGIN TRY
		...
		BEGIN
			...
			BEGIN
				SELECT @MODIFIED_ROWS_PERCENTAGE = (@EMP_STAGING_ROWCOUNT - @SISWEB_OWNER_ROWCOUNT) / @SISWEB_OWNER_ROWCOUNT;
			END;
			ELSE
			BEGIN
			...
			BEGIN
				...
				PRINT FORMATMESSAGE('@EMP_STAGING_ROWCOUNT=%s',FORMAT(@EMP_STAGING_ROWCOUNT,'N','en-us'));
				PRINT FORMATMESSAGE('@MODIFIED_ROWS_PERCENTAGE=%s',FORMAT(@MODIFIED_ROWS_PERCENTAGE,'P','en-us'));
			END;
            /*
			-- If the row count difference between SISWEB_OWNER and EMP_STAGING is greater than 10%, logs an error -> returns FALSE
