...
-- Description: Conditional load from STAGING SIS2NPRPARTHISTORY
-- =============================================
CREATE PROCEDURE SISWEB_OWNER_STAGING.SIS2NPRPARTHISTORY_Merge
...
BEGIN
	...
	BEGIN TRY
		...
		IF @FORCE_LOAD = 'FALSE'
		BEGIN
            EXEC @MODIFIED_ROWS_PERCENTAGE = [SISWEB_OWNER_STAGING].[_getModifiedRowsPercentage] @SISWEB_OWNER_TABLE='SIS2NPRPARTHISTORY', @SISWEB_OWNER_STAGING_TABLE = 'SIS2NPRPARTHISTORY'
		END;
		...
		BEGIN
			...
			/* MERGE command */
			MERGE INTO SISWEB_OWNER.SIS2NPRPARTHISTORY tgt
			USING SISWEB_OWNER_STAGING.SIS2NPRPARTHISTORY src
			ON
			   src.PARTNUMBER = tgt.PARTNUMBER
			   AND src.HISTORYCHAIN = tgt.HISTORYCHAIN
			/* Can't happen */
			...
			OUTPUT $ACTION
				  ...
				  ,COALESCE(inserted.HISTORYCHAIN,deleted.HISTORYCHAIN) HISTORYCHAIN
				   INTO @MERGE_RESULTS;
			/* MERGE command */
			...
			SET @LOGMESSAGE = IIF(@DEBUG = 'TRUE',(SELECT(SELECT COUNT(*) FROM @MERGE_RESULTS AS MR WHERE MR.ACTIONTYPE = 'INSERT') AS Inserted
														...
														,(SELECT MR.ACTIONTYPE
																,MR.PARTNUMBER
																,MR.HISTORYCHAIN FROM @MERGE_RESULTS AS MR FOR JSON AUTO) AS           Modified_Rows FOR JSON PATH,WITHOUT_ARRAY_WRAPPER),'Modified Rows');
		...
		COMMIT;
				...
				UPDATE STATISTICS SISWEB_OWNER.SIS2NPRPARTHISTORY WITH FULLSCAN;
