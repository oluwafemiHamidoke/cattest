...
-- Description: Conditional load from STAGING LNKMEDIAARRANGEMENT
-- =============================================
CREATE PROCEDURE SISWEB_OWNER_STAGING.LNKMEDIAARRANGEMENT_Insert_Update_Delete (@FORCE_LOAD BIT = 'FALSE'
																			   ,@DEBUG      BIT = 'FALSE') 
AS
BEGIN
	...
	BEGIN TRY
		DECLARE @SCHEMANAME sysname = 'SISWEB_OWNER_STAGING'
			   ,@TABLENAME  sysname = 'LNKMEDIAARRANGEMENT';
...
*/
		...
		SELECT CT.SYS_CHANGE_OPERATION AS OPERATION,CT.MEDIANUMBER AS MEDIANUMBER,CT.ARRANGEMENTNO AS ARRANGEMENTNO,CT.SYS_CHANGE_VERSION AS SYS_CHANGE_VERSION
		--,CT.SYS_CHANGE_COLUMNS AS   SYS_CHANGE_COLUMNS -- not needed all columns except LASTMODIFIEDDATE in PK
		INTO #DIFF_PRIMARY_KEYS
		FROM CHANGETABLE(CHANGES SISWEB_OWNER_STAGING.LNKMEDIAARRANGEMENT,@LAST_SYNCHRONIZATION_VERSION) AS CT;
		...
		BEGIN
			...
			BEGIN
				...
				IF @ROWCOUNT > 0
				BEGIN
					DELETE FROM SISWEB_OWNER.LNKMEDIAARRANGEMENT
					OUTPUT 'DELETE' ACTIONTYPE,DELETED.MEDIANUMBER,DELETED.ARRANGEMENTNO,DELETED.LASTMODIFIEDDATE
						   INTO @MERGE_RESULTS
					WHERE EXISTS
					(
						SELECT *
						FROM #DIFF_PRIMARY_KEYS AS DK
						WHERE DK.MEDIANUMBER = LNKMEDIAARRANGEMENT.MEDIANUMBER AND 
							  DK.ARRANGEMENTNO = LNKMEDIAARRANGEMENT.ARRANGEMENTNO AND 
							  DK.OPERATION = 'D'
					);
				...
				IF @ROWCOUNT > 0
				BEGIN
					INSERT INTO SISWEB_OWNER.LNKMEDIAARRANGEMENT (MEDIANUMBER,ARRANGEMENTNO,LASTMODIFIEDDATE) 
					OUTPUT 'INSERT' ACTIONTYPE,INSERTED.MEDIANUMBER,INSERTED.ARRANGEMENTNO,INSERTED.LASTMODIFIEDDATE
						   INTO @MERGE_RESULTS
						   SELECT MEDIANUMBER,ARRANGEMENTNO,@SYSUTCDATETIME
						   FROM SISWEB_OWNER_STAGING.LNKMEDIAARRANGEMENT AS DT
						   WHERE EXISTS
						   (
							   SELECT *
							   FROM #DIFF_PRIMARY_KEYS AS DK
							   WHERE DK.MEDIANUMBER = DT.MEDIANUMBER AND 
									 DK.ARRANGEMENTNO = DT.ARRANGEMENTNO AND 
									 DK.OPERATION = 'I'
						   );
...
/*				-- Can't happen all column in PK
				...
				IF @ROWCOUNT > 0
				BEGIN
					UPDATE SISWEB_OWNER.LNKMEDIAARRANGEMENT
					...
					OUTPUT 'UPDATE' ACTIONTYPE
						  ...
						  ,INSERTED.SNR
						   INTO @MERGE_RESULTS
					  FROM SISWEB_OWNER_STAGING.LNKMEDIAARRANGEMENT AS ST
						   JOIN SISWEB_OWNER.LNKMEDIAARRANGEMENT AS TT ON
...
*/
				...
				(
					...
					(
						SELECT MR.ACTIONTYPE,MR.MEDIANUMBER,MR.ARRANGEMENTNO,MR.LASTMODIFIEDDATE
		...
		BEGIN
			...
			/* MERGE command */
			MERGE INTO SISWEB_OWNER.LNKMEDIAARRANGEMENT tgt
			USING SISWEB_OWNER_STAGING.LNKMEDIAARRANGEMENT src
			ON src.MEDIANUMBER = tgt.MEDIANUMBER AND 
			   src.ARRANGEMENTNO = tgt.ARRANGEMENTNO
			...
			WHEN NOT MATCHED BY TARGET
				  THEN
				  INSERT(MEDIANUMBER,ARRANGEMENTNO,LASTMODIFIEDDATE)
				  VALUES (src.MEDIANUMBER,src.ARRANGEMENTNO,@SYSUTCDATETIME) 
			WHEN NOT MATCHED BY SOURCE
				  THEN DELETE
			OUTPUT $ACTION,COALESCE(inserted.MEDIANUMBER,deleted.MEDIANUMBER) MEDIANUMBER,COALESCE(inserted.ARRANGEMENTNO,deleted.ARRANGEMENTNO) ARRANGEMENTNO,COALESCE(inserted.
		...
		COMMIT;
								...
								UPDATE STATISTICS SISWEB_OWNER.LNKMEDIAARRANGEMENT WITH FULLSCAN;
