CREATE PROCEDURE util.IndexOptimize @Databases                       NVARCHAR(MAX) = NULL
...
BEGIN
	...
	DECLARE @SelectedAvailabilityGroups TABLE
		(AvailabilityGroupName NVARCHAR(MAX)
		,StartPosition         INT
		,Selected              BIT);
	...
	DECLARE @CurrentAlterIndexWithClauseArguments TABLE
		(ID       INT IDENTITY
	...
	DECLARE @CurrentUpdateStatisticsWithClauseArguments TABLE
		(ID       INT IDENTITY
		,Argument NVARCHAR(MAX)
		,Added    BIT DEFAULT 0);
	...
	DECLARE @AmazonRDS BIT = CASE
								 WHEN
									  DB_ID('rdsadmin') IS NOT NULL
	...
	BEGIN
		...
		SELECT 'The database ' + QUOTENAME(DB_NAME(DB_ID())) + ' has to be in compatibility level 90 or higher.'
			  ,16
			  ,1;
	END;
	...
	BEGIN
		...
		SELECT 'The stored procedure CommandExecute is missing. Download https://ola.hallengren.com/scripts/CommandExecute.sql.'
			  ,16
			  ,1;
	END;
	...
	IF EXISTS(SELECT *
				...
				WHERE
					  ...
					  AND schemas.name = 'util'
					  AND objects.name = 'CommandExecute'
					  AND OBJECT_DEFINITION(objects.object_id) NOT LIKE '%@DatabaseContext%')
	BEGIN
		INSERT INTO @Errors
			   (Message
			   ,Severity
		...
		SELECT 'The stored procedure CommandExecute needs to be updated. Download https://ola.hallengren.com/scripts/CommandExecute.sql.'
			  ,16
			  ,1;
	END;
	...
	BEGIN
		...
		SELECT 'The table Queue is missing. Download https://ola.hallengren.com/scripts/Queue.sql.'
			  ,16
			  ,1;
	END;
	...
	IF
	   ...
	   AND NOT EXISTS(SELECT *
						...
						WHERE
							  objects.type = 'U'
							  AND schemas.name = 'util'
							  AND objects.name = 'QueueDatabase')
	BEGIN
		INSERT INTO @Errors
			   (Message
			   ,Severity
	...
	WITH Databases1(StartPosition
		 ...
		 Databases3(DatabaseItem
				   ...
				   ,Selected)
		 AS (SELECT CASE
						WHEN DatabaseItem IN('ALL_DATABASES','SYSTEM_DATABASES','USER_DATABASES','AVAILABILITY_GROUP_DATABASES') THEN '%'
					ELSE DatabaseItem
					END AS DatabaseItem
				   ,CASE
					...
					END AS DatabaseType
				   ,CASE
						WHEN DatabaseItem = 'AVAILABILITY_GROUP_DATABASES' THEN 1
						   ELSE NULL
					END AS AvailabilityGroup
				   ,StartPosition
				   ,Selected
			   FROM Databases2),
		 Databases4(DatabaseName
				   ,DatabaseType
				   ,AvailabilityGroup
				   ,StartPosition
				   ,Selected)
		 ...
		 SELECT DatabaseName
			   ...
			   ,StartPosition
			   ,Selected
		   FROM Databases4 OPTION(MAXRECURSION 0);

	--IF
	--   @Version >= 11
	--   AND SERVERPROPERTY('IsHadrEnabled') = 1
	--BEGIN
	--	INSERT INTO @tmpAvailabilityGroups
	--		   (AvailabilityGroupName
	--		   ,Selected
	--		   ) 
	--	SELECT name AS AvailabilityGroupName
	--		  ,0 AS    Selected FROM sys.availability_groups;

	--	INSERT INTO @tmpDatabasesAvailabilityGroups
	--		   (DatabaseName
	--		   ,AvailabilityGroupName
	--		   ) 
	...
	--		   INNER JOIN sys.dm_hadr_availability_replica_states AS dm_hadr_availability_replica_states ON databases.replica_id = dm_hadr_availability_replica_states.replica_id
	--		   INNER JOIN sys.availability_groups AS availability_groups ON dm_hadr_availability_replica_states.group_id = availability_groups.group_id;
	--END;
	...
	UPDATE tmpDatabases
		   SET AvailabilityGroup = CASE
									   WHEN EXISTS(SELECT * FROM @tmpDatabasesAvailabilityGroups WHERE DatabaseName = tmpDatabases.DatabaseName) THEN 1
								   ELSE 0
								   END
	...
	UPDATE tmpDatabases
	  ...
	  FROM @tmpDatabases tmpDatabases
		   INNER JOIN @SelectedDatabases SelectedDatabases ON
															  tmpDatabases.DatabaseName LIKE REPLACE(SelectedDatabases.DatabaseName,'_','[_]')
															  AND (
																  tmpDatabases.DatabaseType = SelectedDatabases.DatabaseType
																  OR SelectedDatabases.DatabaseType IS NULL)
															  AND (
																  tmpDatabases.AvailabilityGroup = SelectedDatabases.AvailabilityGroup
																  OR SelectedDatabases.AvailabilityGroup IS NULL)
	...
	UPDATE tmpDatabases
	  ...
	  FROM @tmpDatabases tmpDatabases
		   INNER JOIN(SELECT tmpDatabases.DatabaseName
							,MIN(SelectedDatabases.StartPosition) AS StartPosition
						FROM @tmpDatabases AS tmpDatabases
							 INNER JOIN @SelectedDatabases AS SelectedDatabases ON
																				   tmpDatabases.DatabaseName LIKE REPLACE(SelectedDatabases.DatabaseName,'_','[_]')
																				   AND (
																					   tmpDatabases.DatabaseType = SelectedDatabases.DatabaseType
																					   OR SelectedDatabases.DatabaseType IS NULL)
																				   AND (
																					   ...
																					   OR SelectedDatabases.AvailabilityGroup IS NULL)
						WHERE SelectedDatabases.Selected = 1
						GROUP BY tmpDatabases.DatabaseName) SelectedDatabases2 ON tmpDatabases.DatabaseName = SelectedDatabases2.DatabaseName;

	IF
	   @Databases IS NOT NULL
	   AND (
		   NOT EXISTS(SELECT * FROM @SelectedDatabases)
		   OR EXISTS(SELECT * FROM @SelectedDatabases WHERE
	...
	BEGIN
		...
		SELECT 'The value for the parameter @Databases is not supported.'
			  ,16
			  ,1;
	END;
	...
	IF
	   @AvailabilityGroups IS NOT NULL
	   AND @Version >= 11
	   AND SERVERPROPERTY('IsHadrEnabled') = 1
	BEGIN
		...
		WHILE CHARINDEX(@StringDelimiter + ' ',@AvailabilityGroups) > 0
			SET @AvailabilityGroups = REPLACE(@AvailabilityGroups,@StringDelimiter + ' ',@StringDelimiter);
		WHILE CHARINDEX(' ' + @StringDelimiter,@AvailabilityGroups) > 0
			SET @AvailabilityGroups = REPLACE(@AvailabilityGroups,' ' + @StringDelimiter,@StringDelimiter);

		SET @AvailabilityGroups = LTRIM(RTRIM(@AvailabilityGroups));

		WITH AvailabilityGroups1(StartPosition
								,EndPosition
			 ...
			 AvailabilityGroups2(AvailabilityGroupItem
								,StartPosition
								,Selected)
			 AS (SELECT CASE
							WHEN AvailabilityGroupItem LIKE '-%' THEN RIGHT(AvailabilityGroupItem,LEN(AvailabilityGroupItem) - 1)
						ELSE AvailabilityGroupItem
						END AS AvailabilityGroupItem
					   ,StartPosition
					   ,CASE
							WHEN AvailabilityGroupItem LIKE '-%' THEN 0
						ELSE 1
						END AS Selected
				   FROM AvailabilityGroups1),
			 AvailabilityGroups3(AvailabilityGroupItem
								,StartPosition
								,Selected)
			 AS (SELECT CASE
							WHEN AvailabilityGroupItem = 'ALL_AVAILABILITY_GROUPS' THEN '%'
						ELSE AvailabilityGroupItem
						END AS AvailabilityGroupItem
					   ,StartPosition
					   ,Selected
				   FROM AvailabilityGroups2),
			 AvailabilityGroups4(AvailabilityGroupName
								,StartPosition
								,Selected)
			 AS (SELECT CASE
							WHEN
								 LEFT(AvailabilityGroupItem,1) = '['
								 AND RIGHT(AvailabilityGroupItem,1) = ']' THEN PARSENAME(AvailabilityGroupItem,1)
						ELSE AvailabilityGroupItem
						END AS AvailabilityGroupItem
					   ,StartPosition
					   ,Selected
				   FROM AvailabilityGroups3)
			 INSERT INTO @SelectedAvailabilityGroups
					(AvailabilityGroupName
					,StartPosition
					,Selected
					) 
			 SELECT AvailabilityGroupName
				   ,StartPosition
				   ,Selected FROM AvailabilityGroups4 OPTION(MAXRECURSION 0);
		...
		UPDATE tmpAvailabilityGroups
		  ...
		  FROM @tmpAvailabilityGroups tmpAvailabilityGroups
			   INNER JOIN @SelectedAvailabilityGroups SelectedAvailabilityGroups ON tmpAvailabilityGroups.AvailabilityGroupName LIKE REPLACE(SelectedAvailabilityGroups.AvailabilityGroupName,'_','[_]')
		...
		UPDATE tmpAvailabilityGroups
		  ...
		  FROM @tmpAvailabilityGroups tmpAvailabilityGroups
			   INNER JOIN(SELECT tmpAvailabilityGroups.AvailabilityGroupName
							...
							FROM @tmpAvailabilityGroups AS tmpAvailabilityGroups
								 INNER JOIN @SelectedAvailabilityGroups AS SelectedAvailabilityGroups ON tmpAvailabilityGroups.AvailabilityGroupName LIKE REPLACE(SelectedAvailabilityGroups.AvailabilityGroupName,'_','[_]')
							WHERE SelectedAvailabilityGroups.Selected = 1
							GROUP BY tmpAvailabilityGroups.AvailabilityGroupName) SelectedAvailabilityGroups2 ON tmpAvailabilityGroups.AvailabilityGroupName = SelectedAvailabilityGroups2.AvailabilityGroupName;
	...
	IF
	   @AvailabilityGroups IS NOT NULL
	   AND (
		   NOT EXISTS(SELECT * FROM @SelectedAvailabilityGroups)
		   OR EXISTS(SELECT * FROM @SelectedAvailabilityGroups WHERE
																	 AvailabilityGroupName IS NULL
																	 OR AvailabilityGroupName = '')
		   OR @Version < 11
	...
	BEGIN
		...
		SELECT 'The value for the parameter @AvailabilityGroups is not supported.'
			  ,16
			  ,1;
	END;
	...
	IF
	   @Databases IS NULL
	   AND @AvailabilityGroups IS NULL
	BEGIN
		INSERT INTO @Errors
			   (Message
			   ,Severity
		...
		SELECT 'You need to specify one of the parameters @Databases and @AvailabilityGroups.'
			  ,16
			  ,2;
	END;
	...
	IF
	   @Databases IS NOT NULL
	   AND @AvailabilityGroups IS NOT NULL
	BEGIN
		INSERT INTO @Errors
			   (Message
			   ,Severity
		...
		SELECT 'You can only specify one of the parameters @Databases and @AvailabilityGroups.'
			  ,16
			  ,3;
	END;
	...
	WHILE CHARINDEX(@StringDelimiter + ' ',@Indexes) > 0
		SET @Indexes = REPLACE(@Indexes,@StringDelimiter + ' ',@StringDelimiter);
	WHILE CHARINDEX(' ' + @StringDelimiter,@Indexes) > 0
		SET @Indexes = REPLACE(@Indexes,' ' + @StringDelimiter,@StringDelimiter);

	SET @Indexes = LTRIM(RTRIM(@Indexes));

	WITH Indexes1(StartPosition
				 ,EndPosition
				 ,IndexItem)
		 AS (SELECT 1 AS                                                                                                    StartPosition
		 ...
		 SELECT DatabaseName
			   ...
			   ,StartPosition
			   ,Selected
		   FROM Indexes4 OPTION(MAXRECURSION 0);
	...
	WITH FragmentationLow(StartPosition
						 ,EndPosition
						 ,[Action])
		 AS (SELECT 1 AS                                                                                                                               StartPosition
	...
	WITH FragmentationMedium(StartPosition
		 ...
		 SELECT 'Medium' AS                    FragmentationGroup
			   ,ROW_NUMBER() OVER(
				ORDER BY StartPosition ASC) AS Priority
			   ,[Action]
		   FROM FragmentationMedium OPTION(MAXRECURSION 0);

	SET @FragmentationHigh = REPLACE(@FragmentationHigh,@StringDelimiter + ' ',@StringDelimiter);

	WITH FragmentationHigh(StartPosition
						  ,EndPosition
						  ,[Action])
		 AS (SELECT 1 AS                                                                                                                                  StartPosition
		 ...
		 SELECT 'High' AS                      FragmentationGroup
			   ,ROW_NUMBER() OVER(
				ORDER BY StartPosition ASC) AS Priority
			   ,[Action]
		   FROM FragmentationHigh OPTION(MAXRECURSION 0);
	...
	BEGIN
		...
		SELECT 'The value for the parameter @FragmentationLow is not supported.'
			  ,16
			  ,1;
	END;
	...
	BEGIN
		...
		SELECT 'The value for the parameter @FragmentationMedium is not supported.'
			  ,16
			  ,1;
	END;
	...
	BEGIN
		...
		SELECT 'The value for the parameter @FragmentationHigh is not supported.'
			  ,16
			  ,1;
	END;
	...
	BEGIN
		...
		SELECT 'The value for the parameter @FragmentationLevel1 is not supported.'
			  ,16
			  ,1;
	END;
	...
	BEGIN
		...
		SELECT 'The value for the parameter @FragmentationLevel2 is not supported.'
			  ,16
			  ,1;
	END;
	...
	BEGIN
		...
		SELECT 'The value for the parameter @MinNumberOfPages is not supported.'
			  ,16
			  ,1;
	END;
	...
	BEGIN
		...
		SELECT 'The value for the parameter @MaxNumberOfPages is not supported.'
			  ,16
			  ,1;
	END;
	...
	BEGIN
		...
		SELECT 'The value for the parameter @SortInTempdb is not supported.'
			  ,16
			  ,1;
	END;
	...
	BEGIN
		...
		SELECT 'The value for the parameter @MaxDOP is not supported.'
			  ,16
			  ,1;
	END;
	...
	BEGIN
		...
		SELECT 'The value for the parameter @FillFactor is not supported.'
			  ,16
			  ,1;
	END;
	...
	BEGIN
		...
		SELECT 'The value for the parameter @PadIndex is not supported.'
			  ,16
			  ,1;
	END;
	...
	BEGIN
		...
		SELECT 'The value for the parameter @LOBCompaction is not supported.'
			  ,16
			  ,1;
	END;
	...
	BEGIN
		...
		SELECT 'The value for the parameter @UpdateStatistics is not supported.'
			  ,16
			  ,1;
	END;
	...
	IF
	   @OnlyModifiedStatistics NOT IN('Y','N')
	...
	BEGIN
		...
		SELECT 'The value for the parameter @OnlyModifiedStatistics is not supported.'
			  ,16
			  ,1;
	END;
	...
	IF
	   ...
	   OR @StatisticsModificationLevel > 100
	BEGIN
		INSERT INTO @Errors
			   (Message
			   ,Severity
		...
		SELECT 'The value for the parameter @StatisticsModificationLevel is not supported.'
			  ,16
			  ,1;
	END;
	...
	IF
	   ...
	   AND @StatisticsModificationLevel IS NOT NULL
	BEGIN
		INSERT INTO @Errors
			   (Message
			   ,Severity
		...
		SELECT 'You can only specify one of the parameters @OnlyModifiedStatistics and @StatisticsModificationLevel.'
			  ,16
			  ,1;
	END;
	...
	BEGIN
		...
		SELECT 'The value for the parameter @StatisticsSample is not supported.'
			  ,16
			  ,1;
	END;
	...
	BEGIN
		...
		SELECT 'The value for the parameter @StatisticsResample is not supported.'
			  ,16
			  ,1;
	END;
	...
	BEGIN
		...
		SELECT 'The value for the parameter @PartitionLevel is not supported.'
			  ,16
			  ,1;
	END;
	...
	BEGIN
		...
		SELECT 'The value for the parameter @MSShippedObjects is not supported.'
			  ,16
			  ,1;
	END;
	...
	BEGIN
		...
		SELECT 'The value for the parameter @Indexes is not supported.'
			  ,16
			  ,1;
	END;
	...
	IF
	   @Indexes IS NOT NULL
	   AND NOT EXISTS(SELECT * FROM @SelectedIndexes)
	BEGIN
		INSERT INTO @Errors
			   (Message
			   ,Severity
	...
	BEGIN
		...
		SELECT 'The value for the parameter @TimeLimit is not supported.'
			  ,16
			  ,1;
	END;
	...
	BEGIN
		...
		SELECT 'The value for the parameter @Delay is not supported.'
			  ,16
			  ,1;
	END;
	...
	BEGIN
		...
		SELECT 'The value for the parameter @WaitAtLowPriorityMaxDuration is not supported.'
			  ,16
			  ,1;
	END;
	...
	BEGIN
		...
		SELECT 'The value for the parameter @WaitAtLowPriorityAbortAfterWait is not supported.'
			  ,16
			  ,1;
	END;
	...
	BEGIN
		...
		SELECT 'The parameters @WaitAtLowPriorityMaxDuration and @WaitAtLowPriorityAbortAfterWait can only be used together.'
			  ,16
			  ,1;
	END;
	...
	IF
	   @Resumable NOT IN('Y','N')
	   OR @Resumable IS NULL
	BEGIN
		INSERT INTO @Errors
			   (Message
			   ,Severity
		...
		SELECT 'The value for the parameter @Resumable is not supported.'
			  ,16
			  ,1;
	END;
	...
	IF
	   ...
	   AND NOT(
			  @Version >= 14
			  OR SERVERPROPERTY('EngineEdition') IN(5,8))
	BEGIN
		INSERT INTO @Errors
			   (Message
			   ,Severity
	...
	BEGIN
		...
		SELECT 'The value for the parameter @LockTimeout is not supported.'
			  ,16
			  ,1;
	END;
	...
	BEGIN
		...
		SELECT 'The value for the parameter @LockMessageSeverity is not supported.'
			  ,16
			  ,1;
	END;
	...
	IF
	   @StringDelimiter IS NULL
	   OR LEN(@StringDelimiter) > 1
	BEGIN
		INSERT INTO @Errors
			   (Message
			   ,Severity
		...
		SELECT 'The value for the parameter @StringDelimiter is not supported.'
			  ,16
			  ,1;
	END;
	...
	IF
	   @DatabasesInParallel NOT IN('Y','N')
	   OR @DatabasesInParallel IS NULL
	BEGIN
		INSERT INTO @Errors
			   (Message
			   ,Severity
		...
		SELECT 'The value for the parameter @DatabasesInParallel is not supported.'
			  ,16
			  ,1;
	END;
	...
	BEGIN
		...
		SELECT 'The documentation is available at https://ola.hallengren.com/sql-server-index-and-statistics-maintenance.html.'
			  ,16
			  ,1;
	END;
	...
	BEGIN
		...
		SELECT 'The following databases in the @Databases parameter do not exist: ' + LEFT(@ErrorMessage,LEN(@ErrorMessage) - 1) + '.'
			  ,10
			  ,1;
	END;
	...
	BEGIN
		...
		SELECT 'The following databases in the @Indexes parameter do not exist: ' + LEFT(@ErrorMessage,LEN(@ErrorMessage) - 1) + '.'
			  ,10
			  ,1;
	END;
	...
	SELECT @ErrorMessage = @ErrorMessage + QUOTENAME(AvailabilityGroupName) + ', '
	  ...
	  WHERE
			AvailabilityGroupName NOT LIKE '%[%]%'
	...
	BEGIN
		...
		SELECT 'The following availability groups do not exist: ' + LEFT(@ErrorMessage,LEN(@ErrorMessage) - 1) + '.'
			  ,10
			  ,1;
	END;
	...
	BEGIN
		...
		SELECT 'The following databases have been selected in the @Indexes parameter, but not in the @Databases or @AvailabilityGroups parameters: ' + LEFT(@ErrorMessage,LEN(@ErrorMessage) - 1) + '.'
			  ,10
			  ,1;
	END;
	...
	DECLARE ErrorCursor CURSOR FAST_FORWARD
	FOR SELECT Message
			  ,Severity
			  ,[State] FROM @Errors ORDER BY ID ASC;

	OPEN ErrorCursor;

	FETCH ErrorCursor INTO @CurrentMessage
						  ,@CurrentSeverity
						  ,@CurrentState;
	...
	BEGIN
		...
		FETCH NEXT FROM ErrorCursor INTO @CurrentMessage
										,@CurrentSeverity
										,@CurrentState;
	END;
	...
	SET @PartitionLevelStatistics = CASE
										WHEN
											 @PartitionLevel = 'Y'
											 AND (
												 @Version >= 12.05
												 AND @Version < 13
												 OR @Version >= 13.04422
												 OR SERVERPROPERTY('EngineEdition') IN(5,8)) THEN 1
									ELSE 0
									END;
	...
	BEGIN
		UPDATE tmpDatabases
			   SET DatabaseSize = (SELECT SUM(CAST(size AS BIGINT))
									 FROM sys.database_files
									 WHERE
										   type = 0
										   /*AND database_id = DB_ID(tmpDatabases.DatabaseName)*/)
		  FROM @tmpDatabases tmpDatabases;
	END;
		...
		BEGIN
			WITH tmpDatabases
				 AS (SELECT DatabaseName
						   ,[Order]
						   ,ROW_NUMBER() OVER(
							ORDER BY DatabaseName ASC) AS RowNumber
					   FROM @tmpDatabases AS tmpDatabases
					   WHERE Selected = 1)
				 UPDATE tmpDatabases SET [Order] = RowNumber;
		END;
			...
			BEGIN
				WITH tmpDatabases
					 AS (SELECT DatabaseName
							   ,[Order]
							   ,ROW_NUMBER() OVER(
								ORDER BY DatabaseName DESC) AS RowNumber
						   FROM @tmpDatabases AS tmpDatabases
						   WHERE Selected = 1)
					 UPDATE tmpDatabases SET [Order] = RowNumber;
			END;
				...
				BEGIN
					WITH tmpDatabases
						 AS (SELECT DatabaseName
								   ,[Order]
								   ,ROW_NUMBER() OVER(
									ORDER BY DatabaseSize ASC) AS RowNumber
							   FROM @tmpDatabases AS tmpDatabases
							   WHERE Selected = 1)
						 UPDATE tmpDatabases SET [Order] = RowNumber;
				END;
					...
					BEGIN
						WITH tmpDatabases
							 AS (SELECT DatabaseName
									   ,[Order]
									   ,ROW_NUMBER() OVER(
										ORDER BY DatabaseSize DESC) AS RowNumber
								   FROM @tmpDatabases AS tmpDatabases
								   WHERE Selected = 1)
							 UPDATE tmpDatabases SET [Order] = RowNumber;
					END;
	...
	BEGIN
		...
		BEGIN TRY
			...
			BEGIN
				...
				SELECT @QueueID = QueueID
				  FROM util.Queue WITH (UPDLOCK,HOLDLOCK)
				  WHERE
						SchemaName = @SchemaName
						AND ObjectName = @ObjectName
						AND Parameters = @Parameters;
			...
			UPDATE Queue
			  ...
			  WHERE
					...
					AND NOT EXISTS(SELECT *
									 FROM util.QueueDatabase AS QueueDatabase
										  INNER JOIN sys.dm_exec_requests ON
																			 QueueDatabase.SessionID = session_id
																			 AND QueueDatabase.RequestID = request_id
																			 AND QueueDatabase.RequestStartTime = start_time
									 WHERE QueueDatabase.QueueID = @QueueID);

			IF @@ROWCOUNT = 1
			BEGIN
				INSERT INTO util.QueueDatabase
					   (QueueID
					   ,DatabaseName
					   ) 
				SELECT @QueueID AS QueueID
				  ...
				  WHERE
						Selected = 1
						AND NOT EXISTS(SELECT * FROM util.QueueDatabase WHERE
																			  DatabaseName = tmpDatabases.DatabaseName
																			  AND QueueID = @QueueID);

				DELETE QueueDatabase
				  FROM util.QueueDatabase QueueDatabase
				  WHERE
						QueueID = @QueueID
						AND NOT EXISTS(SELECT * FROM @tmpDatabases AS tmpDatabases WHERE
			...
			SELECT @QueueStartTime = QueueStartTime FROM util.Queue WHERE QueueID = @QueueID;
		END TRY
		BEGIN CATCH
			IF XACT_STATE() <> 0
			BEGIN
	...
	BEGIN
		...
		BEGIN
			UPDATE QueueDatabase
			  ...
			  WHERE
					...
					AND NOT EXISTS(SELECT *
									 FROM sys.dm_exec_requests
									 WHERE
										   session_id = QueueDatabase.SessionID
										   AND request_id = QueueDatabase.RequestID
										   AND start_time = QueueDatabase.RequestStartTime);
			...
			UPDATE QueueDatabase
			  ...
			  FROM(SELECT TOP 1 DatabaseStartTime
							   ...
							   ,RequestStartTime
							   ,DatabaseName
					 FROM util.QueueDatabase
					 WHERE
						   QueueID = @QueueID
						   AND (
							   DatabaseStartTime < @QueueStartTime
							   OR DatabaseStartTime IS NULL)
						   AND NOT(
								  DatabaseStartTime IS NOT NULL
								  AND DatabaseEndTime IS NULL)
					 ORDER BY DatabaseOrder ASC) QueueDatabase;
		END;
		ELSE
		BEGIN
		...
		BEGIN
			...
			SET @DatabaseMessage = 'Standby: ' + CASE
													 WHEN @CurrentInStandby = 1 THEN 'Yes'
												 ELSE 'No'
												 END;
			RAISERROR('%s',10,1,@DatabaseMessage) WITH NOWAIT;

			SET @DatabaseMessage = 'Updateability: ' + CASE
														   WHEN @CurrentIsReadOnly = 1 THEN 'READ_ONLY'
														   WHEN @CurrentIsReadOnly = 0 THEN 'READ_WRITE'
													   END;
			RAISERROR('%s',10,1,@DatabaseMessage) WITH NOWAIT;
		...
		--IF
		--   @CurrentDatabaseState = 'ONLINE'
		--   AND SERVERPROPERTY('EngineEdition') <> 5
		--BEGIN
		--	IF EXISTS(SELECT *
		--				FROM sys.database_recovery_status
		--				WHERE
		--					  database_id = DB_ID(@CurrentDatabaseName)
		...
		BEGIN
			SET @DatabaseMessage = 'Is accessible: ' + CASE
														   WHEN @CurrentIsDatabaseAccessible = 1 THEN 'Yes'
													   ELSE 'No'
													   END;
			RAISERROR('%s',10,1,@DatabaseMessage) WITH NOWAIT;
		END;
		...
		BEGIN
			...
			BEGIN
				...
				BEGIN
					...
					SET @CurrentCommand = @CurrentCommand + ' WHERE objects.[type] IN(''U'',''V'')' + CASE
																										  WHEN @MSShippedObjects = 'N' THEN ' AND objects.is_ms_shipped = 0'
																									  ELSE ''
				...
				BEGIN
					SET @CurrentCommand = @CurrentCommand + 'SELECT schemas.[schema_id] AS SchemaID' + ', schemas.[name] AS SchemaName' + ', objects.[object_id] AS ObjectID' + ', objects.[name] AS ObjectName' + ', RTRIM(objects.[type]) AS ObjectType' + ', ' + CASE
																																																																		WHEN @Version >= 12 THEN 'tables.is_memory_optimized'
																																																																	ELSE '0'
																																																																	END + ' AS IsMemoryOptimized' + ', NULL AS IndexID, NULL AS IndexName' + ', NULL AS IndexType' + ', NULL AS AllowPageLocks' + ', NULL AS IsImageText' + ', NULL AS IsNewLOB' + ', NULL AS IsFileStream' + ', NULL AS IsColumnStore' + ', NULL AS IsComputed' + ', NULL AS IsTimestamp' + ', NULL AS OnReadOnlyFileGroup' + ', NULL AS ResumableIndexOperation' + ', stats.stats_id AS StatisticsID' + ', stats.name AS StatisticsName' + ', stats.no_recompute AS NoRecompute' + ', ' + CASE
																																																																																																																																																																																								WHEN @Version >= 12 THEN 'stats.is_incremental'
																																																																																																																																																																																							ELSE '0'
																																																																																																																																																																																							END + ' AS IsIncremental' + ', NULL AS PartitionID' + ', ' + CASE
																																																																																																																																																																																																							 WHEN @PartitionLevelStatistics = 1 THEN 'dm_db_incremental_stats_properties.partition_number'
																																																																																																																																																																																																						 ELSE 'NULL'
																																																																																																																																																																																																						 END + ' AS PartitionNumber' + ', NULL AS PartitionCount' + ', 0 AS [Order]' + ', 0 AS Selected' + ', 0 AS Completed' + ' FROM sys.stats stats' + ' INNER JOIN sys.objects objects ON stats.[object_id] = objects.[object_id]' + ' INNER JOIN sys.schemas schemas ON objects.[schema_id] = schemas.[schema_id]' + ' LEFT OUTER JOIN sys.tables tables ON objects.[object_id] = tables.[object_id]';
					...
					SET @CurrentCommand = @CurrentCommand + ' WHERE objects.[type] IN(''U'',''V'')' + CASE
																										  WHEN @MSShippedObjects = 'N' THEN ' AND objects.is_ms_shipped = 0'
																									  ELSE ''
																									  END + ' AND NOT EXISTS(SELECT * FROM sys.indexes indexes WHERE indexes.[object_id] = stats.[object_id] AND indexes.index_id = stats.stats_id)';
				END;
			...
			BEGIN
				UPDATE tmpIndexesStatistics
				  ...
				  FROM @tmpIndexesStatistics tmpIndexesStatistics
					   INNER JOIN @SelectedIndexes SelectedIndexes ON
																	  @CurrentDatabaseName LIKE REPLACE(SelectedIndexes.DatabaseName,'_','[_]')
				...
				UPDATE tmpIndexesStatistics
				  ...
				  FROM @tmpIndexesStatistics tmpIndexesStatistics
					   INNER JOIN(SELECT tmpIndexesStatistics.SchemaName
										,tmpIndexesStatistics.ObjectName
										,tmpIndexesStatistics.IndexName
										,tmpIndexesStatistics.StatisticsName
										,MIN(SelectedIndexes.StartPosition) AS StartPosition
									FROM @tmpIndexesStatistics AS tmpIndexesStatistics
										 INNER JOIN @SelectedIndexes AS SelectedIndexes ON
																						   @CurrentDatabaseName LIKE REPLACE(SelectedIndexes.DatabaseName,'_','[_]')
																						   AND tmpIndexesStatistics.SchemaName LIKE REPLACE(SelectedIndexes.SchemaName,'_','[_]')
																						   AND tmpIndexesStatistics.ObjectName LIKE REPLACE(SelectedIndexes.ObjectName,'_','[_]')
																						   AND COALESCE(tmpIndexesStatistics.IndexName,tmpIndexesStatistics.StatisticsName) LIKE REPLACE(SelectedIndexes.IndexName,'_','[_]')
									WHERE SelectedIndexes.Selected = 1
									GROUP BY tmpIndexesStatistics.SchemaName
											,tmpIndexesStatistics.ObjectName
											,tmpIndexesStatistics.IndexName
											,tmpIndexesStatistics.StatisticsName) SelectedIndexes2 ON
																									  tmpIndexesStatistics.SchemaName = SelectedIndexes2.SchemaName
																									  AND tmpIndexesStatistics.ObjectName = SelectedIndexes2.ObjectName
																									  AND (
																										  tmpIndexesStatistics.IndexName = SelectedIndexes2.IndexName
																										  OR tmpIndexesStatistics.IndexName IS NULL)
																									  AND (
																										  tmpIndexesStatistics.StatisticsName = SelectedIndexes2.StatisticsName
																										  OR tmpIndexesStatistics.StatisticsName IS NULL);
			END;

			WITH tmpIndexesStatistics
				 AS (SELECT SchemaName
						   ,ObjectName
						   ,[Order]
						   ,ROW_NUMBER() OVER(
							ORDER BY ISNULL(ResumableIndexOperation,0) DESC
									,StartPosition ASC
									,SchemaName ASC
									,ObjectName ASC
									,CASE
										 WHEN IndexType IS NULL THEN 1
												ELSE 0
									 END ASC
									,IndexType ASC
									,IndexName ASC
									,StatisticsName ASC
									,PartitionNumber ASC) AS RowNumber
					   FROM @tmpIndexesStatistics AS tmpIndexesStatistics
					   WHERE Selected = 1)
				 UPDATE tmpIndexesStatistics SET [Order] = RowNumber;

			SET @ErrorMessage = '';
			SELECT @ErrorMessage = @ErrorMessage + QUOTENAME(DatabaseName) + '.' + QUOTENAME(SchemaName) + '.' + QUOTENAME(ObjectName) + ', '
			  FROM @SelectedIndexes AS SelectedIndexes
			  WHERE
					...
					AND NOT EXISTS(SELECT *
									 ...
									 WHERE
										   SchemaName = SelectedIndexes.SchemaName
										   AND ObjectName = SelectedIndexes.ObjectName);
			IF @@ROWCOUNT > 0
			BEGIN
			  ...
			  WHERE
					...
					AND NOT EXISTS(SELECT *
									 ...
									 WHERE
										   SchemaName = SelectedIndexes.SchemaName
										   AND ObjectName = SelectedIndexes.ObjectName
										   AND IndexName = SelectedIndexes.IndexName);
			IF @@ROWCOUNT > 0
			BEGIN
			...
			BEGIN
				...
				IF
				   @CurrentPartitionNumber IS NULL
				...
				IF
				   @CurrentIndexID IS NOT NULL
				   AND EXISTS(SELECT * FROM @ActionsPreferred)
				BEGIN
					SET @CurrentCommand = '';

					IF @LockTimeout IS NOT NULL
						SET @CurrentCommand = 'SET LOCK_TIMEOUT ' + CAST(@LockTimeout * 1000 AS NVARCHAR) + '; ';
					...
					END TRY
					BEGIN CATCH
						SET @ErrorMessage = 'Msg ' + CAST(ERROR_NUMBER() AS NVARCHAR) + ', ' + ISNULL(ERROR_MESSAGE(),'') + CASE
																																WHEN ERROR_NUMBER() = 1222 THEN ' The index ' + QUOTENAME(@CurrentIndexName) + ' on the object ' + QUOTENAME(@CurrentDatabaseName) + '.' + QUOTENAME(@CurrentSchemaName) + '.' + QUOTENAME(@CurrentObjectName) + ' is locked. It could not be checked if the index exists.'
																															ELSE ''
																															END;
						SET @Severity = CASE
				...
				IF
				   @CurrentStatisticsID IS NOT NULL
				...
				BEGIN
					...
					SET @CurrentCommand+='IF EXISTS(SELECT * FROM sys.stats stats INNER JOIN sys.objects objects ON stats.[object_id] = objects.[object_id] INNER JOIN sys.schemas schemas ON objects.[schema_id] = schemas.[schema_id] WHERE objects.[type] IN(''U'',''V'')' + CASE
																																																																					WHEN @MSShippedObjects = 'N' THEN ' AND objects.is_ms_shipped = 0'
																																																																				ELSE ''
					...
					BEGIN CATCH
						SET @ErrorMessage = 'Msg ' + CAST(ERROR_NUMBER() AS NVARCHAR) + ', ' + ISNULL(ERROR_MESSAGE(),'') + CASE
																																WHEN ERROR_NUMBER() = 1222 THEN ' The statistics ' + QUOTENAME(@CurrentStatisticsName) + ' on the object ' + QUOTENAME(@CurrentDatabaseName) + '.' + QUOTENAME(@CurrentSchemaName) + '.' + QUOTENAME(@CurrentObjectName) + ' is locked. It could not be checked if the statistics exists.'
																															ELSE ''
																															END;
						SET @Severity = CASE
											WHEN ERROR_NUMBER() IN(1205,1222) THEN @LockMessageSeverity
				...
				BEGIN
					...
					IF
					   @PartitionLevelStatistics = 1
					   AND @CurrentIsIncremental = 1
					BEGIN
					...
					BEGIN TRY
						EXECUTE @CurrentDatabase_sp_executesql @stmt = @CurrentCommand
															  ,@params = N'@ParamObjectID int, @ParamStatisticsID int, @ParamPartitionNumber int, @ParamRowCount bigint OUTPUT, @ParamModificationCounter bigint OUTPUT'
															  ,@ParamObjectID = @CurrentObjectID
															  ,@ParamStatisticsID = @CurrentStatisticsID
															  ,@ParamPartitionNumber = @CurrentPartitionNumber
															  ,@ParamRowCount = @CurrentRowCount OUTPUT
															  ,@ParamModificationCounter = @CurrentModificationCounter OUTPUT;

						IF @CurrentRowCount IS NULL
							SET @CurrentRowCount = 0;
						IF @CurrentModificationCounter IS NULL
							SET @CurrentModificationCounter = 0;
					END TRY
					BEGIN CATCH
						SET @ErrorMessage = 'Msg ' + CAST(ERROR_NUMBER() AS NVARCHAR) + ', ' + ISNULL(ERROR_MESSAGE(),'') + CASE
																																WHEN ERROR_NUMBER() = 1222 THEN ' The statistics ' + QUOTENAME(@CurrentStatisticsName) + ' on the object ' + QUOTENAME(@CurrentDatabaseName) + '.' + QUOTENAME(@CurrentSchemaName) + '.' + QUOTENAME(@CurrentObjectName) + ' is locked. The rows and modification_counter could not be checked.'
																															ELSE ''
																															END;
						SET @Severity = CASE
											WHEN ERROR_NUMBER() IN(1205,1222) THEN @LockMessageSeverity
				...
				IF
				   ...
				   AND (
					   EXISTS(SELECT Priority
									,[Action]
									,COUNT(*) FROM @ActionsPreferred GROUP BY Priority
																			 ,[Action] HAVING COUNT(*) <> 3)
					   OR @MinNumberOfPages > 0
					   OR @MaxNumberOfPages IS NOT NULL)
				BEGIN
					SET @CurrentCommand = '';
					...
					BEGIN TRY
						EXECUTE sp_executesql @stmt = @CurrentCommand
											 ,@params = N'@ParamDatabaseName nvarchar(max), @ParamObjectID int, @ParamIndexID int, @ParamPartitionNumber int, @ParamFragmentationLevel float OUTPUT, @ParamPageCount bigint OUTPUT'
											 ,@ParamDatabaseName = @CurrentDatabaseName
											 ,@ParamObjectID = @CurrentObjectID
											 ,@ParamIndexID = @CurrentIndexID
											 ,@ParamPartitionNumber = @CurrentPartitionNumber
					...
					BEGIN CATCH
						SET @ErrorMessage = 'Msg ' + CAST(ERROR_NUMBER() AS NVARCHAR) + ', ' + ISNULL(ERROR_MESSAGE(),'') + CASE
																																WHEN ERROR_NUMBER() = 1222 THEN ' The index ' + QUOTENAME(@CurrentIndexName) + ' on the object ' + QUOTENAME(@CurrentDatabaseName) + '.' + QUOTENAME(@CurrentSchemaName) + '.' + QUOTENAME(@CurrentObjectName) + ' is locked. The page_count and avg_fragmentation_in_percent could not be checked.'
																															ELSE ''
																															END;
						SET @Severity = CASE
											WHEN ERROR_NUMBER() IN(1205,1222) THEN @LockMessageSeverity
				...
				BEGIN
					SET @CurrentFragmentationGroup = CASE
														 ...
														 WHEN
															  @CurrentFragmentationLevel >= @FragmentationLevel1
				...
				IF
				   ...
				   AND EXISTS(SELECT * FROM @ActionsPreferred)
				   AND (
					   @CurrentPageCount >= @MinNumberOfPages
					   OR @MinNumberOfPages = 0)
				   AND (
					   @CurrentPageCount <= @MaxNumberOfPages
					   OR @MaxNumberOfPages IS NULL)
				...
				BEGIN
					...
					BEGIN
						SELECT @CurrentAction = [Action]
						  FROM @ActionsPreferred
						  WHERE
								FragmentationGroup = @CurrentFragmentationGroup
					...
					BEGIN
						SELECT @CurrentAction = [Action]
						  FROM @ActionsPreferred
						  WHERE Priority = (SELECT MIN(Priority) FROM @ActionsPreferred WHERE [Action] IN(SELECT [Action] FROM @CurrentActionsAllowed));
					END;
				END;
				...
				IF
				   ...
				   AND (
					   ...
					   OR
						  ...
						  AND (
							  @CurrentPartitionNumber = @CurrentPartitionCount
							  OR
								 @PartitionLevelStatistics = 1
								 AND @CurrentIsIncremental = 1))
				BEGIN
				...
				IF
				   ...
				   AND NOT(
						  ...
						  OR SERVERPROPERTY('EngineEdition') IN(5,8))
				   AND (
					   @CurrentStatisticsSample <> 100
					   OR @CurrentStatisticsSample IS NULL)
				BEGIN
				...
				BEGIN
					SET @CurrentComment = 'ObjectType: ' + CASE
															   WHEN @CurrentObjectType = 'U' THEN 'Table'
															   WHEN @CurrentObjectType = 'V' THEN 'View'
														   ELSE 'N/A'
														   END + ', ';
					SET @CurrentComment+='IndexType: ' + CASE
															 WHEN @CurrentIndexType = 1 THEN 'Clustered'
															 WHEN @CurrentIndexType = 2 THEN 'NonClustered'
															 WHEN @CurrentIndexType = 3 THEN 'XML'
															 WHEN @CurrentIndexType = 4 THEN 'Spatial'
															 WHEN @CurrentIndexType = 5 THEN 'Clustered Columnstore'
															 WHEN @CurrentIndexType = 6 THEN 'NonClustered Columnstore'
															 WHEN @CurrentIndexType = 7 THEN 'NonClustered Hash'
														 ELSE 'N/A'
														 END + ', ';
					SET @CurrentComment+='ImageText: ' + CASE
															 WHEN @CurrentIsImageText = 1 THEN 'Yes'
															 WHEN @CurrentIsImageText = 0 THEN 'No'
														 ELSE 'N/A'
														 END + ', ';
					SET @CurrentComment+='NewLOB: ' + CASE
														  WHEN @CurrentIsNewLOB = 1 THEN 'Yes'
														  WHEN @CurrentIsNewLOB = 0 THEN 'No'
													  ELSE 'N/A'
													  END + ', ';
					SET @CurrentComment+='FileStream: ' + CASE
															  WHEN @CurrentIsFileStream = 1 THEN 'Yes'
															  WHEN @CurrentIsFileStream = 0 THEN 'No'
														  ELSE 'N/A'
														  END + ', ';
					IF @Version >= 11
						SET @CurrentComment+='ColumnStore: ' + CASE
																   WHEN @CurrentIsColumnStore = 1 THEN 'Yes'
																   WHEN @CurrentIsColumnStore = 0 THEN 'No'
															   ELSE 'N/A'
															   END + ', ';
					IF
					   @Version >= 14
					   AND @Resumable = 'Y'
						SET @CurrentComment+='Computed: ' + CASE
																WHEN @CurrentIsComputed = 1 THEN 'Yes'
																WHEN @CurrentIsComputed = 0 THEN 'No'
															ELSE 'N/A'
															END + ', ';
					IF
					   @Version >= 14
					   AND @Resumable = 'Y'
						SET @CurrentComment+='Timestamp: ' + CASE
																 WHEN @CurrentIsTimestamp = 1 THEN 'Yes'
																 WHEN @CurrentIsTimestamp = 0 THEN 'No'
															 ELSE 'N/A'
															 END + ', ';
					SET @CurrentComment+='AllowPageLocks: ' + CASE
																  WHEN @CurrentAllowPageLocks = 1 THEN 'Yes'
																  WHEN @CurrentAllowPageLocks = 0 THEN 'No'
															  ELSE 'N/A'
															  END + ', ';
				...
				BEGIN
					SET @CurrentExtendedInfo = (SELECT *
												  FROM(SELECT CAST(@CurrentPageCount AS NVARCHAR) AS          PageCount
															 ,CAST(@CurrentFragmentationLevel AS NVARCHAR) AS Fragmentation) AS ExtendedInfo FOR XML RAW('ExtendedInfo'),ELEMENTS);
				END;
				...
				BEGIN
					...
					IF
					   @CurrentIsPartition = 1
					   AND @CurrentResumableIndexOperation = 0
						SET @CurrentCommand+=' PARTITION = ' + CAST(@CurrentPartitionNumber AS NVARCHAR);
					...
					BEGIN
						INSERT INTO @CurrentAlterIndexWithClauseArguments (Argument) 
						SELECT 'ONLINE = ON' + CASE
												   WHEN @WaitAtLowPriorityMaxDuration IS NOT NULL THEN ' (WAIT_AT_LOW_PRIORITY (MAX_DURATION = ' + CAST(@WaitAtLowPriorityMaxDuration AS NVARCHAR) + ', ABORT_AFTER_WAIT = ' + UPPER(@WaitAtLowPriorityAbortAfterWait) + '))'
											   ELSE ''
											   END;
					END;
					...
					IF
					  ...
					  @Version >= 14
					  OR SERVERPROPERTY('EngineEdition') IN(5,8))
					  AND @CurrentAction = 'INDEX_REBUILD_ONLINE'
					  AND @CurrentResumableIndexOperation = 0
					BEGIN
						INSERT INTO @CurrentAlterIndexWithClauseArguments (Argument) 
						SELECT CASE
								   WHEN
										...
										AND @CurrentIsComputed = 0
										AND @CurrentIsTimestamp = 0 THEN 'RESUMABLE = ON'
							   ELSE 'RESUMABLE = OFF'
							   END;
					END;
					...
					BEGIN
						...
						BEGIN
							...
							UPDATE @CurrentAlterIndexWithClauseArguments SET Added = 1 WHERE ID = @CurrentAlterIndexArgumentID;
						END;
					...
					IF @CurrentAlterIndexWithClause IS NOT NULL
						SET @CurrentCommand+=@CurrentAlterIndexWithClause;

					EXECUTE @CurrentCommandOutput = util.CommandExecute @DatabaseContext = @CurrentDatabaseName
					...
					IF @CurrentCommandOutput <> 0
						SET @ReturnCode = @CurrentCommandOutput;
				...
				BEGIN
					SET @CurrentComment = 'ObjectType: ' + CASE
														   ...
														   END + ', ';
					SET @CurrentComment+='IndexType: ' + CASE
															 WHEN @CurrentIndexID IS NOT NULL THEN 'Index'
														 ELSE 'Column'
														 END + ', ';
					IF @CurrentIndexID IS NOT NULL
						SET @CurrentComment+='IndexType: ' + CASE
															 ...
															 ELSE 'N/A'
															 END + ', ';
					SET @CurrentComment+='Incremental: ' + CASE
															   WHEN @CurrentIsIncremental = 1 THEN 'Y'
															   WHEN @CurrentIsIncremental = 0 THEN 'N'
														   ELSE 'N/A'
														   END + ', ';
				...
				IF
				   @CurrentStatisticsID IS NOT NULL
				   AND (
					   @CurrentRowCount IS NOT NULL
					   OR @CurrentModificationCounter IS NOT NULL)
				BEGIN
					SET @CurrentExtendedInfo = (SELECT *
												  FROM(SELECT CAST(@CurrentRowCount AS NVARCHAR) AS            [RowCount]
															 ,CAST(@CurrentModificationCounter AS NVARCHAR) AS ModificationCounter) AS ExtendedInfo FOR XML RAW('ExtendedInfo'),ELEMENTS);
				END;
				...
				BEGIN
					...
					IF
					   @CurrentStatisticsSample IS NOT NULL
					   AND @CurrentStatisticsSample <> 100
					BEGIN
						INSERT INTO @CurrentUpdateStatisticsWithClauseArguments (Argument) 
					...
					IF @CurrentUpdateStatisticsWithClause IS NOT NULL
						SET @CurrentCommand+=@CurrentUpdateStatisticsWithClause;

					IF
					   @PartitionLevelStatistics = 1
					   AND @CurrentIsIncremental = 1
					   AND @CurrentPartitionNumber IS NOT NULL
						SET @CurrentCommand+=' ON PARTITIONS(' + CAST(@CurrentPartitionNumber AS NVARCHAR(MAX)) + ')';
		...
		ELSE
		BEGIN
			UPDATE @tmpDatabases SET Completed = 1 WHERE
														 Selected = 1
														 AND Completed = 0
														 AND ID = @CurrentDBID;
		END;
