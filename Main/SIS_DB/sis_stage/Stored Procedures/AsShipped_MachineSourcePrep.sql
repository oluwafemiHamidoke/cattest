...
-- Description: Full load [sis_stage].[AsShipped_MachineSourcePrep]]
--Exec [sis_stage].[AsShipped_MachineSourcePrep]
-- =============================================
CREATE PROCEDURE [sis_stage].[AsShipped_MachineSourcePrep]
--(
...
--SNP / SNR
Update [SISWEB_OWNER].[LNKASSHIPPEDPRODUCTDETAILS] set SNP = left(ltrim(rtrim(SERIALNUMBER)), 3) where SNP is null
Update [SISWEB_OWNER].[LNKASSHIPPEDPRODUCTDETAILS] set SNR = try_cast(Substring(ltrim(rtrim(SERIALNUMBER)), 4, 5) as int) where SNR is null

--Attachement SNP / SNR
--Update [SISWEB_OWNER].[LNKASSHIPPEDPRODUCTDETAILS] set AttachmentSNP = left(ltrim(rtrim(ATTACHMENTSERIALNUMBER)), 3) 
...
--Update [SISWEB_OWNER].[LNKASSHIPPEDPRODUCTDETAILS] set AttachmentSNR = try_cast(Substring(ltrim(rtrim(ATTACHMENTSERIALNUMBER)), 4, 5) as int) 
...
--19 seconds to set 0 values
--Not setting 1 values.  Null values are valid.
Update s
Set isValidSerialNumber = 0
--Select Distinct SERIALNUMBER
From [SISWEB_OWNER].[LNKASSHIPPEDPRODUCTDETAILS] s
...
update [SISWEB_OWNER].[LNKASSHIPPEDPRODUCTDETAILS] set [ID_Int] = cast(ID as bigint) where [ID_Int] is null
...
--Create Index
If NOT EXISTS (SELECT * FROM sys.indexes WHERE name='LNKASSHIPPEDPRODUCTDETAILS_PARENTPARTNUMBER_NC' AND object_id = OBJECT_ID('SISWEB_OWNER.LNKASSHIPPEDPRODUCTDETAILS'))
CREATE NONCLUSTERED INDEX [LNKASSHIPPEDPRODUCTDETAILS_PARENTPARTNUMBER_NC] ON [SISWEB_OWNER].[LNKASSHIPPEDPRODUCTDETAILS]
(
	[SERIALNUMBER] ASC,
	[PARTNUMBER] ASC,
...
Insert into [sis_stage].[Log] ([ProcessID],[LogDateTime],[LogType],[NameofSproc],[LogMessage],[DataValue])
Values (@ProcessID, GETDATE(), 'Information', @ProcName, 'Index LNKASSHIPPEDPRODUCTDETAILS_PARENTPARTNUMBER_NC', @@RowCount)


--Add PartentID
--Source does not update records.  Only delete and insert.  So, no need to recalculate the parentID.
Update c
set ParentID = p.ID_Int
--SELECT c.ID_Int, p.ID_Int ParentID
FROM [SISWEB_OWNER].[LNKASSHIPPEDPRODUCTDETAILS] c
Inner join [SISWEB_OWNER].[LNKASSHIPPEDPRODUCTDETAILS] p on p.PARTNUMBER = c.PARENTPARTNUMBER and c.SERIALNUMBER = p.SERIALNUMBER
where c.PARENTPARTNUMBER is not null and c.ParentID is null
