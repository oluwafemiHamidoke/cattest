...
-- Description: Full load [sis_stage].[AsShipped_MachineSource3Prep]]
--Exec [sis_stage].[AsShipped_MachineSource3Prep]
-- =============================================
CREATE PROCEDURE [sis_stage].[AsShipped_MachineSource3Prep]
--(
...
--SNP / SNR
Update [SISWEB_OWNER].[LNKASSHIPPEDPRODUCTDETAILS3] set SNP = left(ltrim(rtrim(SERIALNUMBER)), 3) where SNP is null
Update [SISWEB_OWNER].[LNKASSHIPPEDPRODUCTDETAILS3] set SNR = try_cast(Substring(ltrim(rtrim(SERIALNUMBER)), 4, 5) as int) where SNR is null
...
--4 seconds to set 0 values
--Not setting 1 values.  Null values are valid.
Update s
Set isValidSerialNumber = 0
--Select Distinct SERIALNUMBER
From [SISWEB_OWNER].[LNKASSHIPPEDPRODUCTDETAILS3] s
...
update [SISWEB_OWNER].[LNKASSHIPPEDPRODUCTDETAILS3] set [ID_Int] = cast(ID as bigint) where [ID_Int] is null
...
--Create Index
If NOT EXISTS (SELECT * FROM sys.indexes WHERE name='LNKASSHIPPEDPRODUCTDETAILS3_PARENTPARTNUMBER_NC' AND object_id = OBJECT_ID('SISWEB_OWNER.LNKASSHIPPEDPRODUCTDETAILS3'))
CREATE NONCLUSTERED INDEX [LNKASSHIPPEDPRODUCTDETAILS3_PARENTPARTNUMBER_NC] ON [SISWEB_OWNER].[LNKASSHIPPEDPRODUCTDETAILS3]
(
	[SERIALNUMBER] ASC,
	[PARTNUMBER] ASC,
...
Insert into [sis_stage].[Log] ([ProcessID],[LogDateTime],[LogType],[NameofSproc],[LogMessage],[DataValue])
Values (@ProcessID, GETDATE(), 'Information', @ProcName, 'Index LNKASSHIPPEDPRODUCTDETAILS3_PARENTPARTNUMBER_NC', @@RowCount)
...
FROM [SISWEB_OWNER].[LNKASSHIPPEDPRODUCTDETAILS3] c
Inner join [SISWEB_OWNER].[LNKASSHIPPEDPRODUCTDETAILS2] p on c.PARENTPARTNUMBER = CONCAT(p.PARENTPARTNUMBER, p.PARTNUMBER) and c.SERIALNUMBER = p.SERIALNUMBER
