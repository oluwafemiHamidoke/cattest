...
-- Modify Date: 20190606 - Davide, changed language tables
-- Description: Full load [sis_stage].Language
--Exec [sis_stage].Language_Load
-- =============================================
CREATE PROCEDURE [sis_stage].[Language_Load]
...
--Load
Insert into [sis_stage].[Language]
...
EXEC sis_stage.WriteLog @PROCESSID = @ProcessID, @LOGTYPE = 'Information', @NAMEOFSPROC = @ProcName, @LOGMESSAGE = 'Language Load', @DATAVALUE = @@RowCount;

--Insert natural keys into key table
Insert into [sis_stage].[Language_Key] (Language_Tag)
Select s.Language_Tag
From [sis_stage].[Language] s
...
--Surrogate Update
EXEC sis_stage.WriteLog @PROCESSID = @ProcessID, @LOGTYPE = 'Information', @NAMEOFSPROC = @ProcName, @LOGMESSAGE = 'Language Update Surrogate', @DATAVALUE = @@RowCount;
