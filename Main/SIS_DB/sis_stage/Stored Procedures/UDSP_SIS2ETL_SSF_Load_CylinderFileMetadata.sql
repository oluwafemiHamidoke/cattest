CREATE PROCEDURE [sis_stage].[UDSP_SIS2ETL_SSF_Load_CylinderFileMetadata] (@cylinderFileMetadata [varchar](MAX))
AS
BEGIN
	...
	BEGIN TRANSACTION;
		...
		SELECT * INTO #fileMetadata FROM openjson(replace(@cylinderFileMetadata, '\', ''))
		with (
		...
		SELECT [fileRid], @INFOTYPE_ID, @PN_TYPE, SUBSTRING([fileName], 0, 10), SUBSTRING([fileName], 0, 10) from #insertRecords

		INSERT INTO sis_stage.ServiceFile_SearchCriteria_Syn(ServiceFile_ID, InfoType_ID, Search_Type, [Value], Search_Value)
		SELECT [fileRid], @INFOTYPE_ID, @SN_TYPE, SUBSTRING([fileName], 10, 11), SUBSTRING([fileName], 10, 11) from #insertRecords
