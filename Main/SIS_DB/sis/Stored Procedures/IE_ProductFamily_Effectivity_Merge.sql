...
-- Description: Merge external table [sis_stage].[IE_ProductFamily_Effectivity] into [sis].[IE_ProductFamily_Effectivity]
				No updates.  All fields are part of the composite PKey.
-- Exec [sis].[IE_ProductFamily_Effectivity_Merge]
====================================================================================================================================================== */
CREATE PROCEDURE [sis].[IE_ProductFamily_Effectivity_Merge] ( @DEBUG BIT = 'FALSE' )
AS
	BEGIN
		SET NOCOUNT ON;
		...
		--BEGIN TRY
			...
			MERGE [sis].[IE_ProductFamily_Effectivity] AS tgt
			USING [sis_stage].[IE_ProductFamily_Effectivity] AS src
			on  tgt.IE_ID = src.IE_ID and tgt.Media_ID = src.Media_ID and tgt.ProductFamily_ID = src.ProductFamily_ID
			WHEN NOT MATCHED BY TARGET
			THEN
				INSERT (IE_ID, ProductFamily_ID, Media_ID) 
				VALUES (src.IE_ID, src.ProductFamily_ID, src.Media_ID)
			...
			SET @LOGMESSAGE = IIF(@DEBUG = 'TRUE',(SELECT(SELECT COUNT(*) FROM @MERGE_RESULTS AS MR WHERE MR.ACTIONTYPE = 'INSERT') AS Inserted
																...
																,(SELECT MR.ACTIONTYPE
																	...
																	,WITHOUT_ARRAY_WRAPPER),'IE_ProductFamily_Effectivity Modified Rows');
