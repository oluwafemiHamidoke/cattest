...
-- Description: Merge external table [sisweb_owner].[LNKNPRINFORECTYPE23] into [sis].[NprInfoRecType23]
-- =============================================
CREATE PROCEDURE [sis].[NprInfoRecType23_Merge]
(@DEBUG      BIT = 'FALSE')
AS
BEGIN
    SET NOCOUNT ON
...
MERGE [sis].[NprInfoRecType23] AS x
    USING [SISWEB_OWNER].[LNKNPRINFORECTYPE23] AS s
    ON (s.PRIMARYSEQNO = x.PRIMARYSEQNO and s.RECORDTYPE = x.RECORDTYPE and s.SEQUENCENUMBER = x.SEQUENCENUMBER )
    WHEN MATCHED AND EXISTS
    (
    SELECT s.COLUMNDATA, s.NPRINDICATOR
    EXCEPT
    SELECT x.COLUMNDATA, x.NPRINDICATOR
    )
    THEN
UPDATE SET COLUMNDATA = s.COLUMNDATA, NPRINDICATOR = s.NPRINDICATOR
    WHEN NOT MATCHED BY TARGET
    THEN
INSERT(PRIMARYSEQNO, RECORDTYPE, SEQUENCENUMBER, COLUMNDATA, NPRINDICATOR)
VALUES(s.PRIMARYSEQNO, s.RECORDTYPE, s.SEQUENCENUMBER, s.COLUMNDATA, s.NPRINDICATOR)
...
SET @LOGMESSAGE = IIF(@DEBUG = 'TRUE',(SELECT(SELECT COUNT(*) FROM @MERGE_RESULTS AS MR WHERE MR.ACTIONTYPE = 'INSERT') AS Inserted
    ...
    ,(SELECT MR.ACTIONTYPE
        ...
        ,WITHOUT_ARRAY_WRAPPER),'NprInfoRecType23 Modified Rows');
...
Update STATISTICS sis.NprInfoRecType23 with fullscan
