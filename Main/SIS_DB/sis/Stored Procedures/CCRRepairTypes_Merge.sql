...
-- Create Date: 20230428
-- Description: Merge table [sis_stage].[CCRRepairTypes] into [sis].[CCRRepairTypes]
-- =============================================
CREATE PROCEDURE [sis].[CCRRepairTypes_Merge]
(@DEBUG      BIT = 'FALSE')
AS
BEGIN
    SET NOCOUNT ON
	...
	BEGIN TRY
		...
		MERGE [sis].[CCRRepairTypes] AS x
			USING [sis_stage].[CCRRepairTypes] AS s
			ON (s.[RepairTypeIndicator] = x.[RepairTypeIndicator] and s.SequenceNumber = x.SequenceNumber)
			WHEN MATCHED AND EXISTS
			(
			SELECT s.[CCRRepairTypes_ID] 
			EXCEPT
			SELECT x.[CCRRepairTypes_ID]
			)
			THEN
		UPDATE SET x.[CCRRepairTypes_ID]	= s.[CCRRepairTypes_ID]
			WHEN NOT MATCHED BY TARGET
			THEN
		INSERT([CCRRepairTypes_ID],[RepairTypeIndicator], SequenceNumber)
		VALUES(s.[CCRRepairTypes_ID], s.[RepairTypeIndicator], s.SequenceNumber)
			OUTPUT $ACTION,
		...
		Update STATISTICS [sis].[CCRRepairTypes] with fullscan
