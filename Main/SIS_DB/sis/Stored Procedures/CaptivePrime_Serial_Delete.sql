...
-- =============================================
CREATE PROCEDURE sis.CaptivePrime_Serial_Delete (@FORCE_LOAD BIT = 'FALSE' -- not used, for future enhancements and consistency with other procs
											   ,@DEBUG      BIT = 'FALSE') 
AS
BEGIN
	...
	BEGIN TRY
		...
		BEGIN TRANSACTION;
		EXEC sis.WriteLog @PROCESSID = @PROCESSID,@LOGTYPE = 'Information',@NAMEOFSPROC = @PROCNAME,@LOGMESSAGE = 'CaptivePrime_Serial Load started',@DATAVALUE = NULL;

		BEGIN
			SET @LOGMESSAGE = 'Executing load: ' + IIF(@FORCE_LOAD = 1,'Force Load = TRUE','Force Load = FALSE');
			EXEC sis.WriteLog @PROCESSID = @PROCESSID,@LOGTYPE = 'Information',@NAMEOFSPROC = @PROCNAME,@LOGMESSAGE = @LOGMESSAGE,@DATAVALUE = NULL;
			...
			WITH CaptivePrime_Serial_VALUES
				 ...
				 --   THEN
				 --   INSERT(Media_ID,Prime_SerialNumber,Captive_SerialNumber,Prime_SerialNumberPrefix_ID,Captive_SerialNumberPrefix_ID,Attachment_Type)
				 --   VALUES (src.Media_ID,src.Prime_SerialNumber,src.Captive_SerialNumber,src.Prime_SerialNumberPrefix_ID,src.Captive_SerialNumberPrefix_ID,src.Attachment_Type) 
				 WHEN NOT MATCHED BY SOURCE
					   THEN DELETE
				 -- should not happen as prefix is a function of the serial number, but just in case
				 --WHEN MATCHED AND EXISTS
				 --(
				 -- SELECT src.Prime_SerialNumberPrefix_ID,src.Captive_SerialNumberPrefix_ID,src.Attachment_Type
				 -- EXCEPT
				 -- SELECT tgt.Prime_SerialNumberPrefix_ID,tgt.Captive_SerialNumberPrefix_ID,tgt.Attachment_Type
				 --)
				 --   THEN UPDATE SET tgt.Prime_SerialNumberPrefix_ID = src.Prime_SerialNumberPrefix_ID,tgt.Captive_SerialNumberPrefix_ID = src.Captive_SerialNumberPrefix_ID,tgt.
				 --   Attachment_Type = src.Attachment_Type
				 OUTPUT $ACTION,COALESCE(inserted.Media_ID,deleted.Media_ID) Media_ID,
				     COALESCE(inserted.Prime_SerialNumber,deleted.Prime_SerialNumber) Prime_SerialNumber,
				     COALESCE(inserted.Captive_SerialNumber,deleted.Captive_SerialNumber) Captive_SerialNumber,
				     COALESCE(inserted.Prime_SerialNumberPrefix_ID,deleted.Prime_SerialNumberPrefix_ID) Prime_SerialNumberPrefix_ID,
				     COALESCE(inserted.Captive_SerialNumberPrefix_ID,deleted.Captive_SerialNumberPrefix_ID) Captive_SerialNumberPrefix_ID,
		...
		EXEC sis.WriteLog @PROCESSID = @PROCESSID,@LOGTYPE = 'Information',@NAMEOFSPROC = @PROCNAME,@LOGMESSAGE = 'CaptivePrime_Serial Load completed',@DATAVALUE = NULL;
	END TRY
	BEGIN CATCH
