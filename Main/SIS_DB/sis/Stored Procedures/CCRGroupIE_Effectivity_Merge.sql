...
-- Description: Merge external table [sis_stage].[CCRGroupIE_Effectivity] into [sis].[CCRGroupIE_Effectivity]
-- =============================================

CREATE PROCEDURE [sis].[CCRGroupIE_Effectivity_Merge]
(@DEBUG      BIT = 'FALSE')
AS
BEGIN
    SET NOCOUNT ON
	...
	BEGIN TRY
		...
		MERGE [sis].[CCRGroupIE_Effectivity] AS x
			USING [sis_stage].[CCRGroupIE_Effectivity] AS s
		...
		SET @LOGMESSAGE = IIF(@DEBUG = 'TRUE',(SELECT(SELECT COUNT(*) FROM @MERGE_RESULTS AS MR WHERE MR.ACTIONTYPE = 'INSERT') AS Inserted
				...
				,(SELECT MR.ACTIONTYPE
					...
					,WITHOUT_ARRAY_WRAPPER),'CCRGroupIE_Effectivity Modified Rows');
		...
		Update STATISTICS [sis].[CCRGroupIE_Effectivity] with fullscan
