...
-- Create Date: 20230314
-- Description: Merge external table [SISWEB_OWNER_STAGING].[LNKFILECPI] into [sis].[ServiceFile_CPI]
-- =============================================
CREATE PROCEDURE [sis].[ServiceFile_CPI_Merge]
(@DEBUG      BIT = 'FALSE')
AS
BEGIN
    SET NOCOUNT ON
    ...
    BEGIN TRY
         ...
         MERGE [sis].[ServiceFile_CPI] AS tgt
            USING [SISWEB_OWNER_STAGING].[LNKFILECPI] AS src
            ON (src.FILERID = tgt.ServiceFile_ID and src.CPINumber = tgt.CPINumber)
            WHEN NOT MATCHED BY TARGET
            THEN
               INSERT (ServiceFile_ID, CPINumber )
               VALUES (src.FILERID, src.CPINumber)
        ...
        ,WITHOUT_ARRAY_WRAPPER),'ServiceFile_CPI Modified Rows');
