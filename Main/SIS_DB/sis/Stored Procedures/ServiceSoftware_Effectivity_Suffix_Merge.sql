CREATE PROCEDURE [sis].[ServiceSoftware_Effectivity_Suffix_Merge]
(@DEBUG      BIT = 'FALSE')
AS
BEGIN
    SET NOCOUNT ON
    ...
    BEGIN TRY
        ...
        MERGE [sis].[ServiceSoftware_Effectivity_Suffix] AS tgt
            USING [sis_stage].[ServiceSoftware_Effectivity_Suffix] AS src
        ON (src.ServiceSoftware_Effectivity_ID = tgt.ServiceSoftware_Effectivity_ID
           and src.Suffix = tgt.Suffix)
        WHEN NOT MATCHED BY TARGET THEN
            INSERT (ServiceSoftware_Effectivity_ID, Suffix)
            VALUES (src.ServiceSoftware_Effectivity_ID, src.Suffix)
        ...
        SET @LOGMESSAGE = IIF(@DEBUG = 'TRUE',(SELECT(SELECT COUNT(*) FROM @MERGE_RESULTS AS MR WHERE MR.ACTIONTYPE = 'INSERT') AS Inserted
                                                        ...
                                                        ,(SELECT MR.ACTIONTYPE
                                                            ...
                                                            ,WITHOUT_ARRAY_WRAPPER),'ServiceSoftware_Effectivity_Suffix Modified Rows');
        ...
        Update STATISTICS sis.ServiceSoftware_Effectivity_Suffix with fullscan
