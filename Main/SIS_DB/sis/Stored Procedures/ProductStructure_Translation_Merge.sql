...
-- Description: Merge external table [sis_stage].[ProductStructure_Translation] into [sis].[ProductStructure_Translation]
-- =============================================
CREATE PROCEDURE [sis].[ProductStructure_Translation_Merge]
(@DEBUG      BIT = 'FALSE')
AS
BEGIN
    SET NOCOUNT ON
...
MERGE [sis].[ProductStructure_Translation] AS x
    USING [sis_stage].[ProductStructure_Translation] AS s
    ON (s.ProductStructure_ID = x.ProductStructure_ID and s.Language_ID = x.Language_ID)
...
UPDATE SET x.Description = s.Description
    ...
    THEN
INSERT (ProductStructure_ID, Language_ID, Description)
VALUES (s.ProductStructure_ID, s.Language_ID, s.Description)
    OUTPUT $ACTION,
...
SET @LOGMESSAGE = IIF(@DEBUG = 'TRUE',(SELECT(SELECT COUNT(*) FROM @MERGE_RESULTS AS MR WHERE MR.ACTIONTYPE = 'INSERT') AS Inserted
													...
													,(SELECT MR.ACTIONTYPE
														...
														,WITHOUT_ARRAY_WRAPPER),'ProductStructure Translation Modified Rows');
...
Update STATISTICS sis.ProductStructure_Translation with fullscan
