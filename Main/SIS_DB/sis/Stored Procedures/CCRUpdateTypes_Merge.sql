...
-- Description: Merge external table [sis_stage].[CCRUpdateTypes] into [sis].[CCRUpdateTypes]
-- =============================================
CREATE PROCEDURE [sis].[CCRUpdateTypes_Merge]
(@DEBUG      BIT = 'FALSE')
AS
BEGIN
    SET NOCOUNT ON
	...
	BEGIN TRY
		...
		MERGE [sis].[CCRUpdateTypes] AS x
			USING [sis_stage].[CCRUpdateTypes] AS s
			ON (s.[CCRUpdateTypes_ID] = x.[CCRUpdateTypes_ID])
			WHEN MATCHED AND EXISTS
			(
			SELECT s.[UpdateTypeIndicator], s.[SequenceNumber]
			EXCEPT
			SELECT x.[UpdateTypeIndicator], x.[SequenceNumber]
			)
			THEN
		UPDATE SET x.[UpdateTypeIndicator]	= s.[UpdateTypeIndicator],
				   x.[SequenceNumber]		= s.[SequenceNumber]
			WHEN NOT MATCHED BY TARGET
			THEN
		INSERT([CCRUpdateTypes_ID],[UpdateTypeIndicator], [SequenceNumber])
		VALUES(s.[CCRUpdateTypes_ID], s.[UpdateTypeIndicator], s.[SequenceNumber])
			OUTPUT $ACTION,
		...
		SET @LOGMESSAGE = IIF(@DEBUG = 'TRUE',(SELECT(SELECT COUNT(*) FROM @MERGE_RESULTS AS MR WHERE MR.ACTIONTYPE = 'INSERT') AS Inserted
				...
				,(SELECT MR.ACTIONTYPE
					...
					,WITHOUT_ARRAY_WRAPPER),'CCRUpdateTypes Modified Rows');
		...
		Update STATISTICS [sis].[CCRUpdateTypes] with fullscan
