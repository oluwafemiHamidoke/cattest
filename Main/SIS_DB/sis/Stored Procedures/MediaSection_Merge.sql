...
-- Description: Merge external table [sis_stage].[MediaSection] into [sis].[MediaSection]
-- =============================================
CREATE PROCEDURE [sis].[MediaSection_Merge]
(@DEBUG      BIT = 'FALSE')
AS
BEGIN
    SET NOCOUNT ON
...
MERGE [sis].[MediaSection] AS x
    USING [sis_stage].[MediaSection] AS s
    ON (s.Media_ID = x.Media_ID and s.Section_Number = x.Section_Number)
    WHEN MATCHED AND EXISTS
    (
    SELECT s.MediaSection_ID
    EXCEPT
    SELECT x.MediaSection_ID
    )
    THEN
UPDATE SET x.MediaSection_ID = s.MediaSection_ID
    WHEN NOT MATCHED BY TARGET
    THEN
INSERT (MediaSection_ID, Media_ID, Section_Number)
VALUES (s.MediaSection_ID, s.Media_ID, s.Section_Number)
    OUTPUT $ACTION,
...
SET @LOGMESSAGE = IIF(@DEBUG = 'TRUE',(SELECT(SELECT COUNT(*) FROM @MERGE_RESULTS AS MR WHERE MR.ACTIONTYPE = 'INSERT') AS Inserted
													...
													,(SELECT MR.ACTIONTYPE
														...
														,WITHOUT_ARRAY_WRAPPER),'MediaSection Modified Rows');
...
Update STATISTICS sis.MediaSection with fullscan
