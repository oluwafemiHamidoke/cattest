...
CREATE FUNCTION sis.tvf_ProductHistory_Details_v2(@LANGUAGE_CODE VARCHAR(2)    = 'en'
											   ,@JSON          NVARCHAR(MAX))
...
AS
	BEGIN
		...
		FOR SELECT T1.PRIMARYSEQNO
				  ...
				  ,T1.PACKAGEQUANTITY
				  ,T1.NPRINDICATOR
			  FROM sis.NprInfo AS T1
				   JOIN sis.Part AS P ON T1.PARTNUMBER = P.Part_Number AND P.Org_Code = @DEFAULT_ORGCODE
			...
			FOR
				WITH T23_LINES
					 AS(SELECT 2 AS    RECORDTYPE
						  ...
						  FROM sis.NprInfoRecType23 AS T2
				...
				WITH T456_LINES
					 AS(SELECT _T3.SEQUENCENUMBER
						  ...
						  FROM(SELECT T3.PRIMARYSEQNO
								 ...
								 FROM sis.NprInfoRecType456 AS T3
